...
>
> 16 项特征卡让 AI 认识你的项目，45 个质量门禁守护代码合规——特征卡是立法，门禁是执法。
...
[![Quality Gates](https://img.shields.io/badge/quality%20gates-45-orange)]()
...
| 理念 | 含义 |
|------|------|
| 先认识，再行动 | AI 写代码前必须先认识项目。16 项特征卡完成认知，45 个门禁守护行动 |
| 拼装式开发 | 新功能 = 既有稳定单元拼装 + 最小新增胶水代码。禁止重复造轮子/侵入式重构/破坏性改造 |
| 呈现递进的关系 | 门禁不是"数 import 数"——每个计数背后指向一条关系规律 |
| 特征卡是立法，门禁是执法 | 16 项特征卡定义「项目应该是什么样的」，45 个门禁验证「代码是否符合」 |
...
**特征卡驱动一切：** → 文件填充（SKILL.md 铁律 ← 第 2/6 项、codebase.md ← 第 4 项、reference-manual.md 组件库 ← 第 11 项）→ 门禁配置（precheck.conf 151 个变量从特征卡推导，懒生成机制按 ACTIVE_FRAMEWORKS 自动补占位）→ 开发流程（开始新需求时从第 11 项检索可复用单元）。
...
> **门禁分层（决策 19）：** 45 门禁按 `fail()` 能力分三档，与 core/standard/compliance 三档正交（横切维度）：
> - **strict（14 个）**：≥3 个 fail 调用，真正阻断交付的硬门禁（branch/layer/security/compliance/sbom/privacy/authz/requirements/rtm/release-sign/reuse/shift-left/dengbao/pia）
> - **warn（20 个）**：1-2 个 fail，混合 warn，能 fail 但触发条件窄
> - **advisory（10 个）**：0 个 fail，永不阻断，只 warn/pass（cognition/consistency/consistency-cross/link-depth/state/mermaid/operate/decision-audit/canary/cwe-audit）
>
...
| `--dengbao` | 等保 2.0 控制点（DENGBAO_LEVEL 二/三级分级：双因子/审计日志/审计字段/个人信息保护缺口，启用后 fail-closed + 豁免留痕） | GB/T 22239-2019 |
...
```
自检 → 读取项目知识 → 探查仓库 → 提取 16 项特征卡 → 创建骨架 → 填充全部文件（消除占位符）→ 配置 precheck.conf → 生成 hooks/commands → 运行 45 个门禁 → 写回记忆 → 脚本确认零占位符（`generate-skill.sh --verify-completeness`）
```
...
**自举：** swarm-yuan 能用自身的 45 个门禁检查自身。一个连自己都检查不了的工具，凭什么检查你的项目？
...
│   │   ├── precheck.sh           ← 45 个门禁（3800+ 行，随门禁扩展演进）
│   │   ├── precheck.conf         ← 151 个配置变量模板
...
| **质量门禁** | **45 个（核心 10 + 架构 17 + 合规 13，特征卡立法 + 门禁执法）** |
| 运行时工具 | 11 |
| spec 模板 | 23 主段（§23=发布后运营） |
| 领域知识 | 32 个领域 |
| 认知基底 | 5 层 |
...
> AI 的代码生成能力已经很强，但「项目认知」还停留在零。swarm-yuan 用 16 项特征卡让 AI 先懂你的项目，用 45 个质量门禁守护代码合规——特征卡是立法，门禁是执法。
