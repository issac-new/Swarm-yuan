...
```bash
# >>> swarm-yuan:framework-gate:mybatis >>>
...
- 目标 `precheck.sh` 内设标记区块 `# >>> swarm-yuan:framework-gates >>> ... # <<< swarm-yuan:framework-gates <<<`，注入 = 幂等替换区块内容
- `check_framework()` 调度器的 `case` 分支按 ACTIVE_FRAMEWORKS **重新生成**；未知框架分支从 warn 升级为 **fail**（探查到但没实现 = 范式缺陷，必须暴露）
...
`--framework` 门禁自身两处收紧：未知框架分支 `warn → fail`；ACTIVE_FRAMEWORKS 未配置但探查信号明显（如存在 `*Mapper.xml`）→ warn 提示"疑似漏配 <规则集>"。
