...
| 步骤 | 做什么 |
|------|--------|
| ⓪ | 自检（13 运行时整合口径 = 11 工具机械探测 + 2 方法论引用） |
...
- **流A（生成流程）**：生成器把一个仓库变成目标技能的 12 步流程（Step 0-12，逐步详解在 references/generation-flow.md）。
- **流B（执勤工作流）**：目标技能里的九节点开发工作流（①需求理解→②探查→③设计 spec→④实施 plan→⑤编码→⑥测试→⑦独立审查→⑧合入→⑨发布，载体是目标技能的 references/workflow.md）。
- **档（参考文档）**：references/ 目录下按需阅读的参考文档的简称（"47 档"=47 个 .md 文件；每档开头有"何时读我"路由行）。
- **随发**：随目标技能一起安装分发——generate-skill.sh 的 UNIVERSAL_FILES 清单列出的文件会拷进每个目标技能；不在清单里的文档只存在于生成器仓，目标技能读不到。
- **整合**：一项被吸收的能力"从哪来、在哪个环节被使用、用不了时怎么降级"的登记关系。
- **能力地图（capability-map.md）**：吸收内容的总台账——每个参考文档的来源、证据分级、被使用的环节、何时读；生成器侧维护，不随发。
- **方法论分派表**：references/task-methodology-router.md 里"任务类型 → 该读哪些参考文档"的反查表，随目标技能分发；与能力地图互为正反——地图从文档查用途，分派表从任务查文档。
- **【生成器侧】**：分派表里的标注，表示该文档只在生成器仓里存在、不随目标技能分发（引用它的任务是在生成器里做的）。
- **G 断言（G13-G25 等）**：生成器 self-check 里编号的自检断言；G25=能力地图双向对账（每个参考文档都进台账、台账不记幽灵、路由表与台账互指、任务分派可达、被分派的文档随发或在生成器侧）。
- **三件套（生成期必读面）**：SKILL.md + exploration-guide.md + template-spec.md——跑生成流程前必读的三个文件，合计字节数有预算上限。
- **认知面**：AI 一次会话要读的文档总量（按字节计），分两条预算线：生成期必读三件套、随技能分发总量（UNIVERSAL_FILES 拷贝体积）。
- **税制**：预算的别名——把每次会话的固定阅读成本当作"税"，超出上限必须逐例登记说明理由，不许静默膨胀。
- **变异锁**：证明校验真能拦住错的正反测试——正向：合规样本必须通过；负向：故意注入违规必须被拦。防"断言永远绿但其实拦不住"。
