...
> **阅读方式**：先读 `docs/DESIGN-LOGIC.md`（整体设计主线与层次澄清——四切面统一层次树：生命周期/产物形态/可信度/依赖边界；本文各章是切面的详述）。全文自包含；按需深读用 §11 的证据与档案索引。
...
swarm-yuan 是**元技能生成器**：为任意代码仓库生成项目专属开发技能（"目标技能"）。

**设计主线（完整因果链）**：使命（AI 懂项目再写代码）→ 应用层目的（拼装式开发：基于稳定组件拼装，规避破坏性改造）→ 前提（探查产出"零件目录"：组件/接口/约束全量清单）→ 三支柱（探查可信 / 行为受控 / 随项目演进）→ 体系自身可信（三层 Harness：事中拦截→事后复盘→评测自身）。本文 §1-§8 是这条主线在四个切面（生命周期/产物形态/可信度/依赖边界，见 DESIGN-LOGIC.md）上的详述。
