...
| ocr | alibaba/open-code-review | Apache-2.0 | v1.11.9（2026-09-12 R26 升级；`references/review-methodology.md` R16/R20/R22 补核段） | v1.11.9（2026-09-12） | synced（2026-09-12 R26 升 1.11.9：**--preview 应用与评审相同选择集**（#801——预览=执行口径族第三实例：预览路径须消费与执行路径同一选择函数）+ viewer session compare 页（#1175，纯查看器面不吸收）+ deepseek-flash 模型（#1210）；R24 升 1.11.8：**Rego policy review 支持**（规则面新语言）+ **预览选择与执行对齐**（所见即所扫——展示=执行口径一致族新样本，与 gsd「no-op 报真实条件」同族）+ LLM 流式 usage 默认请求；R22 升 1.11.7：报告**原子写入** + MCP shutdown 阶段限界 + 第二次信号立即退出 + `timeout_sec` 进 config set——稳定性修复族对账通过；R20 一等评审控制/R16 三机制吸收不变） | baseline_status=synced |
| graphify | Graphify-Labs/graphify（原 safishamsi/graphify，已迁移；**2026-09-09 R20 本地克隆 origin 已重指向新 org**） | Apache-2.0（2026-07-18 MIT->Apache 2.0，v0.9.25） | v0.9.58（2026-09-11 R24 升级，origin/v8 线；`references/code-graph-tools.md`） | GitHub v0.9.58（2026-09-10；npm 0.10.0 仍是 2026-06-06 异源旧分支不取，v1.0.0 同） | synced（2026-09-11 R24 升 0.9.58：**python/php/bash import 解析**（跨语言边扩展）+ 同文件源路径碰撞合并修复 + SQL 索引 + rust static/const 声明提取——图谱完整性族延续；R22 升 0.9.57：**增量重建不再擦除跨文件项目 AST 节点**（重提取 `.csproj` 曾连带丢被引用项目节点）+ 重复节点合并保**更富节点**为幸存者 + C# 泛型调用点解析 + `this.X` 成员全形态捕获——图谱完整性修复族对账通过；R20/R18 吸收不变） | baseline_status=synced |
...
| ruflo | ruvnet/ruflo（原 Claude Flow） | MIT | v3.41.1（2026-09-11 R24 升级；`references/subagent-orchestration.md` R16/R20/R22 补核段） | npm 3.41.1（2026-09-10 Latest） | synced（2026-09-11 R24 升 3.41.1：**x.ruv.io 开放 Nostr swarm federation 网关**（MCP + ruv:// + ws proxy）+ **公私 swarm channels**（ADR-386）+ **Seraphina（swarm queen）协调者**——R22 Federation+Claims 候选产品化推进；3.41.1 两口径修复：**截断推理 dump 不得作为协调指导报告**（截断≠结论，与「缺失证据不显示为零」同族）+ **memory 显式路径遵循且单 store 不当全量计数**；R22 升 3.40.0：**Cross-Host Federation + Claims**（跨机协调用 **Ed25519 签名可验证消息 + work claims**；双协调器：本地 mesh agentbbs HTTP pull 固定公钥 / Slack 结构化消息零基建）+ 3.39.3 修复 federation 工具 unconditional degraded 误报与 probe shell 注入——「签名可验证消息+工作认领」跨机编排信任原语**方法论级登记候选**（本仓单机场景无即时落地）；R20 吸收不变；方法论引用层，无运行时调用） | baseline_status=synced |
