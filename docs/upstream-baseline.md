...
| comet | rpamis/comet | MIT | v0.4.0（2026-09-09 R20 升级，§四预登记触发条件命中；`references/subagent-orchestration.md` 0.4.0 能力清单已重写） | 0.4.0（2026-09-08 stable，rc.1-rc.6 收敛后发布） | synced（2026-09-09 R20 兑现 R16 预登记处置：rc.5→0.4.0 收尾 diff 快审**无新增方法论机制**（稳定性收敛）；R16 登记候选维持：Supervisor Change v2 多 session 依赖子图 / Portable State / Agent Learning Loop；已吸收核心理念（脚本背书状态机/无证据不流转/handoff SHA256）在 0.4 全部保留） | baseline_status=synced |
...
| gstack | garrytan/gstack | MIT | v1.84.1.0（2026-09-10 R22 升级，main commit 71f6048；offline-cache vendor `offline-cache/gstack/VERSION:1` 不动） | v1.84.1.0（2026-09-09；无 release 无 tag，VERSION 递增节奏） | synced（2026-09-10 R22 升 1.84.1.0：**impeccable interop**（四个设计技能 detector 前置 + DOM 模式扫描 + 开放 DESIGN.md 格式 + typed slop catalog——方法论引用层内部首次跨对象联动，登记观察）+ Codex/Claude 默认前沿模型；R20 recall bridge/R16/R18 吸收维持） | baseline_status=synced |
| ruflo | ruvnet/ruflo（原 Claude Flow） | MIT | v3.41.2（2026-09-13 R27 升级，tag 2026-09-10 19:45 UTC = 北京 09-11 03:45，npm Latest 已跟进（R24 核测在其 npm 发布前后脚，窗口期错过非漏检）；`references/subagent-orchestration.md` R16/R20/R22 补核段） | npm 3.41.2（2026-09-13 核 Latest） | synced（2026-09-13 R27 升 3.41.2：**内存索引保护**（修复 daemon/memory 销毁用户索引——破坏性操作守卫族：写操作须辨「我的产物 vs 用户资产」）+ **autoStart:false 被尊重**（同意面：显式否决必须生效）+ **Seraphina 预算封顶替代 ad-hoc 计数**（x-gateway 0.6.1——有界性族精化：界从计数上限走向预算分配，与 claude-mem #3575 剩余死线封顶同族）+ 默认频道声明（安静频道仍可发现）；R24 升 3.41.1：**x.ruv.io 开放 Nostr swarm federation 网关**（MCP + ruv:// + ws proxy）+ **公私 swarm channels**（ADR-386）+ **Seraphina（swarm queen）协调者**——R22 Federation+Claims 候选产品化推进；3.41.1 两口径修复：**截断推理 dump 不得作为协调指导报告**（截断≠结论，与「缺失证据不显示为零」同族）+ **memory 显式路径遵循且单 store 不当全量计数**；R22 升 3.40.0：**Cross-Host Federation + Claims**（跨机协调用 **Ed25519 签名可验证消息 + work claims**；双协调器：本地 mesh agentbbs HTTP pull 固定公钥 / Slack 结构化消息零基建）+ 3.39.3 修复 federation 工具 unconditional degraded 误报与 probe shell 注入——「签名可验证消息+工作认领」跨机编排信任原语**方法论级登记候选**（本仓单机场景无即时落地）；R20 吸收不变；方法论引用层，无运行时调用） | baseline_status=synced |
...
| dsh | deepseek-ai/deepseek-harness | MIT | dsh-v0.1.5-rc.2（2026-09-11 R24 升级；`references/dsh-engineering-methodology.md` 四簇吸收 + §七 0.1.1 + §八 0.1.2 + §九 0.1.5） | dsh-v0.1.5-rc.2（2026-09-10；rc 作基线有 R17 先例） | synced（2026-09-11 R24 升 rc.2：web feedback + file refinements 回移 0.1.5 线（2 commits），无方法论新原语；R22 升 rc.1，R20 预告触发点「0.1.5 rc」命中：**动态修改系统提示不破 KV Cache**（模型显式声明支持——缓存友好编排第三实证：系统提示从「不可变前缀」变为「可变但保持命中的受控更新」，技能动态注入可行性上游样本）+ **子代理消息队列与 steer 语义统一**（排队/编辑/删除/Steer/停止；Agent Team `send_message` 统一 steer 且跨 Agent 与冷恢复**保留发送者归属与顺序**——编排「保持不变量」清单再添消息归属项）+ **暂停即终止模型轮次**（模型不能自行恢复，恢复必须用户触发——用户控制权 fail-closed）+ Web 通用文件上传混排（模型按路径按需读取）+ Sidebar 多标签产物预览 + 代理环境变量遵循；R17 跨版本读兼容/删后端纪律/失败词表/Agent Teams 围栏吸收维持） | baseline_status=synced |
