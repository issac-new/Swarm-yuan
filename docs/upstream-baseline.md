...
| gstack | garrytan/gstack | MIT | v1.87.0.0（2026-09-16 R29 升级，main commit 4a3c6a8；offline-cache vendor `offline-cache/gstack/VERSION:1` 不动） | v1.87.0.0（2026-09-16 核；无 release 无 tag，VERSION 递增节奏） | synced（2026-09-10 R22 升 1.84.1.0：**impeccable interop**（四个设计技能 detector 前置 + DOM 模式扫描 + 开放 DESIGN.md 格式 + typed slop catalog——方法论引用层内部首次跨对象联动，登记观察）+ Codex/Claude 默认前沿模型；R20 recall bridge/R16/R18 吸收维持） | baseline_status=synced |
| ruflo | ruvnet/ruflo（原 Claude Flow） | MIT | v3.42.0（2026-09-16 R29 升级，tag + npm Latest 双核；`references/subagent-orchestration.md` R16/R20/R22 补核段 + `references/mcp-governance.md` R29 段） | npm 3.42.0（2026-09-16 核 Latest） | synced（2026-09-13 R27 升 3.41.2：**内存索引保护**（修复 daemon/memory 销毁用户索引——破坏性操作守卫族：写操作须辨「我的产物 vs 用户资产」）+ **autoStart:false 被尊重**（同意面：显式否决必须生效）+ **Seraphina 预算封顶替代 ad-hoc 计数**（x-gateway 0.6.1——有界性族精化：界从计数上限走向预算分配，与 claude-mem #3575 剩余死线封顶同族）+ 默认频道声明（安静频道仍可发现）；R24 升 3.41.1：**x.ruv.io 开放 Nostr swarm federation 网关**（MCP + ruv:// + ws proxy）+ **公私 swarm channels**（ADR-386）+ **Seraphina（swarm queen）协调者**——R22 Federation+Claims 候选产品化推进；3.41.1 两口径修复：**截断推理 dump 不得作为协调指导报告**（截断≠结论，与「缺失证据不显示为零」同族）+ **memory 显式路径遵循且单 store 不当全量计数**；R22 升 3.40.0：**Cross-Host Federation + Claims**（跨机协调用 **Ed25519 签名可验证消息 + work claims**；双协调器：本地 mesh agentbbs HTTP pull 固定公钥 / Slack 结构化消息零基建）+ 3.39.3 修复 federation 工具 unconditional degraded 误报与 probe shell 注入——「签名可验证消息+工作认领」跨机编排信任原语**方法论级登记候选**（本仓单机场景无即时落地）；R20 吸收不变；方法论引用层，无运行时调用） | baseline_status=synced |
