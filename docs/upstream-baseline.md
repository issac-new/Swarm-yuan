...
| claude-code | anthropics/claude-code | 专有（Anthropic Commercial Terms；CLI 二进制分发，源码不开） | v2.1.263（2026-09-06 R18 补核；`references/claude-code-capabilities.md` 全量 159 版调研 + R4/R16/R17/R18 四轮补核） | npm @anthropic-ai/claude-code latest 2.1.263（2026-09-05，npm 无 262 版本号）/ stable 通道 2.1.236（dist-tag 分裂持续，stable 落后 27 patch，2026-09-06 观察） | synced（2026-09-06 R18 补核 v2.1.263 **纯修复轮**（changelog 单条 reliability，无功能性条目，无吸收）；R17 吸收维持：`--permission-prompts none` 无头执法档（**prompt 档无人值守坍缩为 deny**——R13 三值化环境边界注记，双宿主无头对偶）+ **宿主 deny 语义漂移警示**（259 Read-deny 应用至 Bash 参数 260 即回退 + strict sandbox 下 `!` 命令出沙箱——自持门禁兜底教义再实证）+ `/skill-doctor` 技能成本审计（门禁预算宿主侧证据源）+ 子代理提示词外置/输出预算设置（上下文外置第四次验证）+ `CLAUDE_CODE_SUBAGENT_MODEL_FORCE`（adaptive-gating 候选补全）+ Workflow schema 前置校验 + Containment Escape 规则） | baseline_status=synced |
...
| comet | rpamis/comet | MIT | v0.3.9（`references/subagent-orchestration.md:118`） | 0.4.0-rc.5（2026-09-05 同日复核；rc.2-5 全为稳定性修复，正式版未出） | drifted（观望维持——R17 复核：正式版仍未出，R16 裁决不变（rc 阶段 117k 行新子系统 churn + 升级破坏性大，等 stable 一次做对）） | baseline_status=drifted |
...
| ocr | alibaba/open-code-review | Apache-2.0 | v1.11.5（2026-09-05 同日复核升；`references/review-methodology.md` R16 补核段） | v1.11.5（2026-09-05） | synced（R17 升 1.11.4 纯修复对账通过；同日复核 1.11.5：依赖安全补丁（grpc/fast-uri/browserslist 高危通告）+ viewer 评论 fixed/ignored 标记——patch 线对账通过零改动；R16 语义分组/session compare/effort 三机制吸收不变） | baseline_status=synced |
| graphify | Graphify-Labs/graphify（原 safishamsi/graphify，已迁移） | Apache-2.0（2026-07-18 MIT->Apache 2.0，v0.9.25） | v0.9.55（2026-09-06 R18 升，origin/v8 线；`references/code-graph-tools.md`） | GitHub v0.9.55（2026-09-05；npm 0.10.0 仍是 2026-06-06 异源旧分支不取，v1.0.0 同） | synced（R16 升 v0.9.53；同日复核 0.9.54；2026-09-06 R18 升 0.9.55：六项图谱完整性修复——模块 docstring 提取（shebang/license 头后不丢）/ 私有成员 id 盐化防撞 / ghost node 保守合并 / **幻影 type-use 边关闭**（无节点符号不得产边，防幻觉族同向）/ 同名文件标签消歧 / re-export barrel——完整性修复族对账通过不补段） | baseline_status=synced |
...
| gstack | garrytan/gstack | MIT | v1.80.0.0（2026-09-06 R18 升级，main commit c241216；offline-cache vendor `offline-cache/gstack/VERSION:1` 不动） | v1.80.0.0（2026-09-05 VERSION 递增；无 release 无 tag；1.78-1.80 共 5 commits） | synced（2026-09-06 R18 升 1.80.0.0：**1.79 ship 子代理派发不再搁浅整轮运行**（#2772/#2440 族——failure-detector SPINNING 同向，子代理失败面爆炸半径收敛）+ 1.80 hooks 共享单一状态根 + setup 对失败 Chromium 安装存活（wiring 层韧性）+ 1.78 two-red-lanes 质量波（AUQ 塌陷归零/OSV 转绿）；R16 吸收 v1.68.3-1.77 spawned 会话原语 + ponytail 简化 lens + **fail-closed 的 wiring 层也会 fail-open**（PIPESTATUS[0] 钉 wiring 测试）+ 门禁普查反不变式清幻影门禁维持） | baseline_status=synced |
...
| impeccable | pbakaus/impeccable | Apache-2.0 | v4.1.1（2026-08-21 升级；`references/frontend-design-methodology.md`，方法论引用层第 5 对象；Modes/三层权威/craft-floor/59 反模式字典/finish_reviewer 完工审查模式 + v4.1 对抗性 verdict 多候选先内部比较再呈交 + 平台感知验证 + 证据坏则重取证重跑；G13 断言守引用存在性） | skill-v4.2.1（2026-09-06 GitHub 实测，2026-09-05 发布；engine-v0.1.2 线起） | synced（2026-09-05 R17 观察 v4.1.3/v4.2.0：Comps 可执行契约阶段门 + **录制回放钉行为**（830 命令 + 16058 函数调用逐字节比对——与本仓 cli-ab 逐字节等价断言同构）+ 统一规则引擎/无 Node 依赖单二进制——候选级维持，引用基线不升（R16 裁决沿用），候选登记 `references/review-methodology.md` R17/R18 段）；2026-09-06 R18 观察 4.2.1：Windows 引擎时间戳签名 + 下载失败三态区分 + 非交互 Claude 会话技能发现——候选级维持 | baseline_status=synced |
...
2. **版本漂移（1 项 drifted，2026-09-06 R18 重核后）**：comet 0.3.9→0.4.0-rc.5（仍观望——rc.2-5 全为稳定性修复，正式版未出；R16 裁决沿用）。R17（2026-09-05）补核轮把 claude-code/codex-cli/dsh 三件套升基线（dsh 0.1.2 调研兑现 R16 预告）+ openspec/claude-mem/ocr/ruflo/codex-security 5 项升级、GitNexus v1.6.11 stable 登记（详见各条目吸收注记 + §13 历史档案 A15 + `docs/research/R17-runtime-refresh.md`）；R18（2026-09-06）补核：claude-code v2.1.263 纯修复升基线 + gsd-core v1.13.0/graphify v0.9.55/gstack v1.80.0.0 三行升级 + impeccable v4.2.1 候选观察（§13 历史档案 A16 + `docs/research/R18-runtime-refresh.md`）。
