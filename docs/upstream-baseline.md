...
| GitNexus | abhigyanpatwari/GitNexus | **PolyForm Noncommercial 1.0.0**（禁商用，API 返回 NOASSERTION，LICENSE 原文实测） | npm 1.6.9（引用 `context/trace`） | GitHub v1.6.12-rc.13（2026-09-09 R20 观察：rc 线继续，stable 未出；npm 停滞于 2026-06-09） | license-risk（PolyForm 禁商用不变，仍非默认、不追（决策 18）；引用载体 `references/code-graph-tools.md` 许可证表已同步登记） | baseline_status=license-risk |
| gsd-core | open-gsd/gsd-core | MIT | v1.13.0（2026-09-06 R18 升级；`references/gsd-patterns.md` + `references/decision-governance.md` R16/R18 补核段） | npm 1.13.0（2026-09-06） | synced（2026-09-06 R18 升 1.13.0：**证据纪律三连**——复核阻塞须确定性证据（#4085）+ no-op 报真实条件与已算值（#4157，"缺失证据不显示为零"族）+ 不可读目录不得报为空（#4163）+ **Review Dispositions Ledger 契约化**（#4345，与 gate-audit.jsonl 同构）+ context-drift 前置门（#4147，与 fingerprint --diff 同向）+ 候选登记 dispatch.maxConcurrency 容量轴/quick-batch 可恢复 manifest；R16 吸收 `.planning/state.json` 状态契约 + `<fails_when>` 强制 + 共识门证据加权 + 缺失≠VERIFIED + runtime-identity 断言维持） | baseline_status=synced |
