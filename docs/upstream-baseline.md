...
| claude-code | anthropics/claude-code | 专有（Anthropic Commercial Terms；CLI 二进制分发，源码不开） | v2.1.268（2026-09-11 R24 补核；`references/claude-code-capabilities.md` 全量调研 v2.0.73→v2.1.252——GitHub releases + npm 2.x.y 全 223 版，计数口径以其首行为准——+ R4/R16/R17/R18/R20/R22 六轮补核） | npm @anthropic-ai/claude-code latest 2.1.268（2026-09-11 核）/ stable 通道 2.1.236（dist-tag 分裂持续） | synced（2026-09-11 R24 补核 v2.1.268 修复主导实质 patch：**符号链接目录 deny/ask 规则真实路径绕过修复**（路径检查须在规范化空间比对——权限路径语义第六实证；本仓 scope 门字面前缀匹配登记为已知边界）+ **同行不可分析命令（env -C/eval）deny 不再失效**（「不可分析即最坏情况」fail-closed 族延续）+ **WebFetch 300s 宿主死线**（`CLAUDE_CODE_WEBFETCH_DEADLINE_MS` 可覆盖——联网验证工具死线环境事实，生成技能无须自设兜底）+ **机密不落展示面**（plugin/marketplace/MCP 配置 token 与 ${VAR} redaction）；R22 267 实质 patch 吸收维持：**`maxEffortLevel` 设置**（top-level/per-model 封顶 effort——effort 治理进宿主原生配置，与 PreModelSwitch/SUBAGENT_MODEL_FORCE 成谱系，候选登记）+ **prompt-cache 工具动态性大族**（约 12 项 fix：MCP 重连不重写工具表/新工具 deferred definitions/resume 重放录制工具描述——缓存稳定性编排不变量第三波实证，工具面变更由宿主 deferred/replay 消解）+ **managed allow-list 不可读 → deny-all**（fail-closed 第四实证）+ `--system-prompt-snapshot off`（快照 vs 新鲜度显式权衡）；R20 265 实质轮五项吸收维持） | baseline_status=synced |
...
| claude-mem | thedotmack/claude-mem | Apache-2.0 | v13.24.10（2026-09-11 R24 升级；`references/memory-persistence.md` R16 段 + R17 补核） | GitHub v13.24.10（2026-09-10；**npm 通道恢复可安装**（"Fleet can install from npm again"，latest=13.24.5——R20 登记的 npm 回钉 12.4.7 异常缓解；oracle 仍以 GitHub tag 为准）） | watch（迭代极快；R24 升 13.24.10（72 commits）：**凭据泄漏路径关闭**（#3985）+ 配额熔断上浮至 observer-health/session-start 可见（熔断可观测性）+ **CJK/日文 substring 检索** + sync 内容 flush 批次收缩与 hub push 超时上调 + Windows ghost listener 端口探测有界化；R22 升 13.24.5：CCS Align phases 0-3（middle cache/exclude marks/rules walker/sign-off）+ Grok Bot awareness pilot + OpenRouter 每日牌价历史；R17 吸收维持） | baseline_status=watch |
| ocr | alibaba/open-code-review | Apache-2.0 | v1.11.8（2026-09-11 R24 升级；`references/review-methodology.md` R16/R20/R22 补核段） | v1.11.8（2026-09-10） | synced（2026-09-11 R24 升 1.11.8：**Rego policy review 支持**（规则面新语言）+ **预览选择与执行对齐**（所见即所扫——展示=执行口径一致族新样本，与 gsd「no-op 报真实条件」同族）+ LLM 流式 usage 默认请求；R22 升 1.11.7：报告**原子写入** + MCP shutdown 阶段限界 + 第二次信号立即退出 + `timeout_sec` 进 config set——稳定性修复族对账通过；R20 一等评审控制/R16 三机制吸收不变） | baseline_status=synced |
