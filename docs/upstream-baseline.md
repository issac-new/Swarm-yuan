...
| claude-code | anthropics/claude-code | 专有（Anthropic Commercial Terms；CLI 二进制分发，源码不开） | v2.1.266（2026-09-09 R20 补核；`references/claude-code-capabilities.md` 全量 159 版调研 + R4/R16/R17/R18/R20 五轮补核） | npm @anthropic-ai/claude-code latest 2.1.266（2026-09-08）/ stable 通道 2.1.236（dist-tag 分裂持续） | synced（2026-09-09 R20 补核 v2.1.264-266：**2.1.265 实质轮**——`--plugin-dir` 目录化加载（子目录各自加载+运行中热增删）+ 工具结果 1GB 落盘上限（截断显式披露）+ **prompt-cache 稳定性修复族**（resume/teammates 不得重排提示前缀——缓存稳定性成为编排不变量）+ **中断工具调用恢复诚实性**（保留并标记 interrupted，不改写上一条 prompt——partial 语义第二次实证）+ 不可信内容标记（Artifact 读取他人产物标记内嵌指令，候选登记）；266 修复 265 的 `CLAUDE_CODE_USE_GATEWAY` 未文档化变量语义漂移（**宿主行为细节版本间不保证稳定**第三次实证）；R17/R18 吸收维持） | baseline_status=synced |
| codex-cli | openai/codex | Apache-2.0 | rust-v0.153.4（2026-09-05 R17 补核；`references/codex-methodology.md` 省 token 三件套/review rubric/测试哲学执行纪律 + R4/R16/R17 三轮补核） | git tag rust-v0.153.4（2026-09-04）；npm @openai/codex latest 0.153.4（同日复核：npm 通道已追平 git tag）；v0.154.0-alpha.4 在 alpha 通道 | synced（2026-09-05 R17 补核 v0.153.0-153.4（106 commits；0.153.1-4 共 7 个为 GPT-6-Astra 模型目录热修；docs/skills.md 连续第二轮零 diff；**无 v0.150 级破坏项**）；吸收 **Guardian 条件性兜底**（Full Access/User approval 模式跳过评分/评审——执法确定性论证必须落在自持门禁，宿主审批层不保证在场；与 Claude Code 260 deny 回退构成双宿主同向证据）+ `request_user_input_async` 回合中结构化问答原语（Interrupt 守护点候选正式参照，按模型可用性限定）+ hooks 内置白名单三层信任（builtin/managed/user——本仓用户级 PreToolUse 三能力接线对账零变化）+ context_management experimental_mode（ChatGPT 后端限定，登记观望）） | baseline_status=synced |
...
| claude-mem | thedotmack/claude-mem | Apache-2.0 | v13.24.1（2026-09-05 同日复核升；`references/memory-persistence.md` R16 段 + R17 补核） | GitHub v13.24.1（2026-09-05；**npm latest 被回钉 12.4.7**——分发主通道切插件市场 `/plugin install claude-mem@thedotmack`，npm 为遗留/SDK 通道，oracle 以 GitHub tag 为准） | watch（迭代极快；R17 补核 #3838 熔断器事件形状修正（判据须绑定上游真实事件形状——对账通过）+ 13.24.1 插件构建修复（sync-plugin-manifests 覆盖 Cursor/Grok 副本案）对账通过） | baseline_status=watch |
| ocr | alibaba/open-code-review | Apache-2.0 | v1.11.6（2026-09-09 R20 升级；`references/review-methodology.md` R16/R20 补核段） | v1.11.6（2026-09-07） | synced（2026-09-09 R20 升 1.11.6：**一等评审控制**（effort / max_tokens_budget / llm_reasoning_effort 参数化 + live progress——评审成本/深度可配置化上游实证）+ OCaml/ReasonML 支持 + 信号转发 + Existing Code 行号；R16 语义分组/session compare/effort 三机制吸收不变） | baseline_status=synced |
| graphify | Graphify-Labs/graphify（原 safishamsi/graphify，已迁移；**2026-09-09 R20 本地克隆 origin 已重指向新 org**） | Apache-2.0（2026-07-18 MIT->Apache 2.0，v0.9.25） | v0.9.56（2026-09-09 R20 升级，origin/v8 线；`references/code-graph-tools.md`） | GitHub v0.9.56（2026-09-07；npm 0.10.0 仍是 2026-06-06 异源旧分支不取，v1.0.0 同） | synced（2026-09-09 R20 升 0.9.56：Rust trait 方法提取 + Node subpath imports 解析 + 无向 MCP 查询 + TS normalizer 去二次方扫描 + Windows 临时路径边界——覆盖面/性能修复族对账通过；R18 六项图谱完整性修复吸收不变） | baseline_status=synced |
...
| gstack | garrytan/gstack | MIT | v1.83.0.0（2026-09-09 R20 升级，main commit caba78f；offline-cache vendor `offline-cache/gstack/VERSION:1` 不动） | v1.83.0.0（2026-09-09；无 release 无 tag，VERSION 递增节奏） | synced（2026-09-09 R20 升 1.83.0.0：**Memorable recall bridge**（opt-in + 回执式记忆召回桥）；R16/R18 吸收维持（spawned 会话原语/fail-closed 的 wiring 层也会 fail-open/ship 子代理派发不搁浅） | baseline_status=synced |
| ruflo | ruvnet/ruflo（原 Claude Flow） | MIT | v3.38.23（2026-09-09 R20 升级；`references/subagent-orchestration.md` R16/R20 补核段） | npm 3.38.23（2026-09-07 Latest；v3.39.0 tag 无 release 按 oracle 不取） | synced（2026-09-09 R20 升 3.38.23：tier 匹配门控模型路由转发 + embedding-cosine 进 MMR 检索 + **真实冷启动测量替换虚构基准**（诚实性同向）+ ReDoS 边界；R16/R17 吸收不变；方法论引用层，无运行时调用） | baseline_status=synced |
...
2. **版本漂移（0 项 drifted，2026-09-09 R20 重核后）**：comet 0.4.0 正式版发布，R16 预登记触发条件命中，引用基线 v0.3.9→v0.4.0、drifted→synced——**16 行全部 synced/watch/license-risk，drift 归零**。R20（2026-09-09）补核：claude-code v2.1.266（265 实质轮）+ comet 兑现 + ocr/graphify/ruflo/ECC/codex-security/gstack 六行升级（§13 历史档案 A17 + `docs/research/R20-runtime-refresh.md`）；R18（2026-09-06）/R17（2026-09-05）明细见各条目吸收注记与对应档案。
