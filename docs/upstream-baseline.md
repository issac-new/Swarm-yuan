...
| ocr | alibaba/open-code-review | Apache-2.0 | v1.12.4（2026-09-16 R32 升级；`references/review-methodology.md` R16/R20/R22 补核段） | v1.12.4（2026-09-16 R32 核：薄轮——#1310 diff 对超大未跟踪文件**读前跳过**（有界读取族：进入读取前先量体积边界）+ #1288 workspace 模式 untracked binary 判 binary（不可分析输入显式分类，诚实族）+ #1156 action 可选 pr_number 输入 + workflow_run 触发回退（事件源第二通道）；R31 的 1.12.3 安全批——code_execute 路径穿越修复 + 规范化（路径检查须在规范化空间，268 同族）、每环境 .env 保护 + 机密路径排除评审、工具调用 JSON 尾随恢复 + 重复退避；R29 的 1.12.1/2 薄轮——suggestdiff 保留大小写/缩进 + preview 总量圈定 tracked 变更集 + Python stub 路由 + opencode backgroundFile） | synced（2026-09-13 R27 升 1.12.0 **名义 minor 实薄轮**：分组返回文件索引而非路径（#1209 perf 实现面）+ viewer store 会话态（查看器面）+ glm-5.3 通道命名 chore——无新方法论原语，**版本号标签 ≠ 实质判定**；R26 升 1.11.9：**--preview 应用与评审相同选择集**（#801——预览=执行口径族第三实例：预览路径须消费与执行路径同一选择函数）+ viewer session compare 页（#1175，纯查看器面不吸收）+ deepseek-flash 模型（#1210）；R24 升 1.11.8：**Rego policy review 支持**（规则面新语言）+ **预览选择与执行对齐**（所见即所扫——展示=执行口径一致族新样本，与 gsd「no-op 报真实条件」同族）+ LLM 流式 usage 默认请求；R22 升 1.11.7：报告**原子写入** + MCP shutdown 阶段限界 + 第二次信号立即退出 + `timeout_sec` 进 config set——稳定性修复族对账通过；R20 一等评审控制/R16 三机制吸收不变） | baseline_status=synced |
