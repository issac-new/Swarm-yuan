...
| graphify | Graphify-Labs/graphify（原 safishamsi/graphify，已迁移） | Apache-2.0（2026-07-18 MIT->Apache 2.0，v0.9.25） | v0.9.54（2026-09-05 同日复核升，origin/v8 线；`references/code-graph-tools.md`） | GitHub v0.9.54（2026-09-05；npm 0.10.0 仍是 2026-06-06 异源旧分支不取，v1.0.0 同） | synced（R16 升 v0.9.53；同日复核 0.9.54：dup-edge collapse + 原子 version/canvas 写入（os.replace）+ rationale scoring tier + tomli 依赖——原子写入为既有幂等写入吸收面同向增量，对账通过不补段） | baseline_status=synced |
