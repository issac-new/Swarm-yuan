...
| claude-mem | thedotmack/claude-mem | Apache-2.0 | v13.24.5（2026-09-10 R22 升级；`references/memory-persistence.md` R16 段 + R17 补核） | GitHub v13.24.5（2026-09-09；**npm 通道恢复可安装**（"Fleet can install from npm again"，latest=13.24.5——R20 登记的 npm 回钉 12.4.7 异常缓解；oracle 仍以 GitHub tag 为准）） | watch（迭代极快；R22 升 13.24.5：CCS Align phases 0-3（middle cache/exclude marks/rules walker/sign-off）+ Grok Bot awareness pilot + OpenRouter 每日牌价历史；R17 吸收维持） | baseline_status=watch |
| ocr | alibaba/open-code-review | Apache-2.0 | v1.11.7（2026-09-10 R22 升级；`references/review-methodology.md` R16/R20/R22 补核段） | v1.11.7（2026-09-09） | synced（2026-09-10 R22 升 1.11.7：报告**原子写入** + MCP shutdown 阶段限界 + 第二次信号立即退出 + `timeout_sec` 进 config set——稳定性修复族对账通过；R20 一等评审控制/R16 三机制吸收不变） | baseline_status=synced |
| graphify | Graphify-Labs/graphify（原 safishamsi/graphify，已迁移；**2026-09-09 R20 本地克隆 origin 已重指向新 org**） | Apache-2.0（2026-07-18 MIT->Apache 2.0，v0.9.25） | v0.9.57（2026-09-10 R22 升级，origin/v8 线；`references/code-graph-tools.md`） | GitHub v0.9.57（2026-09-09；npm 0.10.0 仍是 2026-06-06 异源旧分支不取，v1.0.0 同） | synced（2026-09-10 R22 升 0.9.57：**增量重建不再擦除跨文件项目 AST 节点**（重提取 `.csproj` 曾连带丢被引用项目节点）+ 重复节点合并保**更富节点**为幸存者 + C# 泛型调用点解析 + `this.X` 成员全形态捕获——图谱完整性修复族对账通过；R20/R18 吸收不变） | baseline_status=synced |
...
| ruflo | ruvnet/ruflo（原 Claude Flow） | MIT | v3.40.0（2026-09-10 R22 升级；`references/subagent-orchestration.md` R16/R20/R22 补核段） | npm 3.40.0（2026-09-09 Latest，GitHub release 同日追平） | synced（2026-09-10 R22 升 3.40.0：**Cross-Host Federation + Claims**（跨机协调用 **Ed25519 签名可验证消息 + work claims**；双协调器：本地 mesh agentbbs HTTP pull 固定公钥 / Slack 结构化消息零基建）+ 3.39.3 修复 federation 工具 unconditional degraded 误报与 probe shell 注入——「签名可验证消息+工作认领」跨机编排信任原语**方法论级登记候选**（本仓单机场景无即时落地）；R20 吸收不变；方法论引用层，无运行时调用） | baseline_status=synced |
...
| codex-security | openai/codex-security | Apache-2.0（完全开源） | npm-v0.1.26（2026-09-09 R20 升级；`references/codex-security-methodology.md`，CLI 接线层第 4 对象；**AI 约束推理扫描（非传统 SAST）** + source→sink 数据流 + 静态评估七元组 + 威胁模型五要素 + scan contract 三件套 + **15 bundled skills**（v0.1.17+ 新增 assess-patch-risk）+ Docker 沙箱范式；`--sast-deep` 门禁 `SAST_DEEP_TOOL=codex-security` 时显式调用，**非降级链一环（非 SAST，auto 降级链不变，两者正交可并行）**；G15 断言守 CLI 接线存在性） | npm-v0.1.26（2026-09-08） | synced（2026-09-09 R20 升 0.1.26：GitLab MR 验证补丁通道 + 安全修复验证须显式请求（fail-closed 同向）+ Daybreak 访问 advisory + confirmed finding 匹配提速；R17 跨扫描发现关系保留吸收不变） | baseline_status=synced |
| dsh | deepseek-ai/deepseek-harness | MIT | dsh-v0.1.5-rc.1（2026-09-10 R22 升级；`references/dsh-engineering-methodology.md` 四簇吸收 + §七 0.1.1 + §八 0.1.2 + §九 0.1.5） | dsh-v0.1.5-rc.1（2026-09-10，跨 0.1.3-alpha→0.1.5-rc 两功能线；rc 作基线有 R17 先例） | synced（2026-09-10 R22 升级，R20 预告触发点「0.1.5 rc」命中：**动态修改系统提示不破 KV Cache**（模型显式声明支持——缓存友好编排第三实证：系统提示从「不可变前缀」变为「可变但保持命中的受控更新」，技能动态注入可行性上游样本）+ **子代理消息队列与 steer 语义统一**（排队/编辑/删除/Steer/停止；Agent Team `send_message` 统一 steer 且跨 Agent 与冷恢复**保留发送者归属与顺序**——编排「保持不变量」清单再添消息归属项）+ **暂停即终止模型轮次**（模型不能自行恢复，恢复必须用户触发——用户控制权 fail-closed）+ Web 通用文件上传混排（模型按路径按需读取）+ Sidebar 多标签产物预览 + 代理环境变量遵循；R17 跨版本读兼容/删后端纪律/失败词表/Agent Teams 围栏吸收维持） | baseline_status=synced |
