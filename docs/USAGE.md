...
2. 根据需求判断变更规模（简单/标准/完整），只引导你填需要的段
...
```bash
# 日常开发：核心 10 门禁（~5 秒）
bash .claude/skills/my-project-dev/scripts/precheck.sh --all

# 架构审查：全部 25 门禁（~30 秒）
bash .claude/skills/my-project-dev/scripts/precheck.sh --all-full
```
...
或对 AI 说"跑安全检查"。所有 25 个门禁都支持单独运行。
