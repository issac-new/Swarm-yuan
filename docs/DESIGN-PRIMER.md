...
**一句话**：swarm-yuan 是一个用 bash 写的"技能生成器"——对着任意一个代码仓库运行一次，它就产出这个项目专属的 AI 编码助手规则包（称为**项目技能**），这个规则包先教 AI 认识这个项目（立法），再在 AI 每次动手改代码之前检查规则（执法）。

**三分钟版**：

1. **问题**：AI 编程助手的短板不在"写代码"，在"不认识项目"——它不知道这个项目的分支规矩、目录分层、哪些文件是稳定资产不能乱碰、改动会影响谁。给 AI 一篇规矩文档没有用，因为文档没有强制力，AI 读完会忘、会糊弄。
2. **解法**：swarm-yuan 把"认识项目"做成一张**特征卡**（17 项探查清单，从项目类型到稳定单元清单——这是立法），再把特征卡翻译成一个可执行的规则包（项目技能），规则包里有一组**门禁**（机器检查脚本，AI 每次改代码前自动运行——这是执法）。立法和执法分离：规矩是数据，检查是机器。
3. **例子**：指着某个 Spring Boot 仓库跑一次生成器，得到的项目技能会知道"这是 Maven 多模块、MyBatis 持久层、master 分支保护"，之后 AI 每次要改代码，门禁会自动检查：分支名合不合规、有没有碰声明为"稳定"的模块、MyBatis 的写法有没有踩这个仓库的坑——不合规就拦下或警告。
...
**② 规矩必须执法，不能只立法**。很多项目有 CONTRIBUTING.md / 开发规范文档，但 AI（以及新人）不读、读不全、读完不遵守。swarm-yuan 的立场：规则只有变成"每次行动前自动运行的机器检查"才存在。所以项目技能里最重要的资产是门禁，不是文档。
...
- **特征卡**（17 项探查清单）= 立法。它回答"这个项目是什么样子的"：项目类型、可改范围、技术栈、构建命令、分支规范、测试体系、稳定单元清单、合规基线……17 项由 AI 探查仓库后填写，探查结果沉淀为项目技能的配置文件（**conf**，一组 KEY=VALUE 变量，当前 176 个）。
...
为什么是分离的？因为规矩随项目变（数据），检查逻辑不随项目变（代码）。同一个门禁函数，在不同项目技能里读不同的 conf，就检查不同的规矩。

> 数字口径：17 项特征卡、176 个 conf 变量、54 个门禁——均以 facts.conf 为准。

### 3.2 生成而非手写：一个生成器，产出 N 个项目技能

swarm-yuan 自己是**生成器**（也叫"元技能"——生成技能的技能）。它对着某个项目跑一次**生成流程**（12 个主步骤，唯一口径见 `swarm-yuan/references/generation-flow.md`），产出该项目的**项目技能**。生成流程是"脚本做机械节点 + AI 做判断节点"的分工：自检、骨架生成、占位符核验由脚本做；探查仓库、填特征卡、配置 conf 由 AI 做（AI 全自动、零手动配置是奠基期铁律——研发人员对 AI 说一句话，而不是自己跑命令）。

项目技能生成出来是 **draft**（草稿态：骨架已建但特征卡未填满，此时禁用全量门禁，防止"半成品看起来很完整"），AI 填充并验证无占位符后转 **active**（激活态，全量门禁解锁）。

### 3.3 重量级显式可选 + 两体系统

本系统不假装轻量。它把"重"拆成两半并各自管理：

- **生成器侧厚**：74 个框架的规则库、40 份方法论文档、生成/自检脚本——这些只在"生成项目技能"时消费一次，厚不构成税。
- **生成物侧薄**：项目技能是被 AI **每次会话**加载的，每字节都是税（稀释 AI 的有效上下文）。所以生成物有硬预算：入口文件 SKILL.md ≤8KB、项目地图 ≤32KiB（预算由 facts.conf 键值定义，生成流水线里有机器实测）。
...
4. **骨架生成**（脚本）：按 profile 档位拷出项目技能骨架（状态=draft）。
...
9. **日常使用**（宿主 hooks + 门禁）：AI 每次写代码前，预检门禁自动跑；写代码时，hooks 拦截对稳定资产/禁区的改动；遇到危险命令（`rm -rf`、`git reset --hard`、`sudo`、`npm publish`），三值规则直接拒绝并给出替代方案；AI 声称"完成了"时，Oracle 机制独立重跑验证命令，通不过就打回继续。
...
| **② 严格度**（enforce 分层，按 fail 调用数归） | strict 16 / warn 23 / advisory 15（合计 54） | "这个门禁会阻断交付吗？" |
...
项目技能挂到 AI 助手的 hooks 上（双宿主：Claude Code 用 deny JSON 协议，Codex 用 exit 2 协议）。五个钩子各司其职：写文件前拦稳定资产/禁区（fail-gate-hook）、防 AI 伪造验证（integrity-guard）、检测失败模式原地打转（failure-detector）、会话结束时独立验证"完成"声明（loop-hook 的 Oracle）、恢复上下文（状态续传）。布局纪律：项目技能里**可执行文件全在 `scripts/` 目录，`hooks/` 目录只放 hooks.json 配置**（曾因装错目录导致钩子全部静默失效，现在有机器断言引用路径必须真实存在）。
...
- **gate-audit.jsonl**：拦截审计流水（每次 hooks 放行/拦截一行）。

关键设计：decisions 账的每行记录引用 trace 账末行的校验和（digest）——三本账从"并列"升级为"链式"：上游任何一行被篡改，下游引用就失配，全链标记为失效。**账本不是日志，是可审计证据**。

### 5.6 生成物税制：每字节都是成本

项目技能被 AI 每次会话加载，体积直接稀释有效上下文。所以生成物有明确预算并由生成流水线实测：入口 SKILL.md ≤8192 字节、项目地图 ≤32KiB、概念负担压到个位数名词。进生成物清单的审核问题永远是："目标 AI 每会话要为它付多少税？"行为参数一律留在包内，不进全局配置。

---

## 6. 端到端：研发人员的一天（使用视角）

1. 新需求来了，对 AI 说"开始新需求 ×××"。项目技能生成 **spec**（任务规格：需求/决策/约束/测试/回滚，23 节模板按任务类型裁减——小任务不必填完整仪式节）。
...
| 易混对 | 辨析 |
|--------|------|
| **生成器 vs 项目技能** | 生成器 = swarm-yuan 本身（造规则包的）；项目技能 = 生成物（某项目专属的规则包）。两者各有一份 `SKILL.md` 和 `scripts/`——生成器的 SKILL.md 是生成器的入口说明书；项目技能的 SKILL.md 是项目技能的入口说明书（≤8KB 预算管的是**后者**）。 |
| **门禁家族 vs 严格度 vs 执行序列** | 见 §5.1。家族=归哪个套餐管；严格度=会不会阻断；执行序列=怎么触发。三轴正交，互不推出。 |
| **运行时三口径** | "13"=外部运行时**整合**总数（深度接线 4 + CLI 接线 4 + 纯方法论引用 5）；"11"=自检**可检测可安装**的工具数（codex-security 无独立包可装、impeccable 是纯方法论，不在 11 内）；"7"=**安装目标**（项目技能可以装进哪几个 AI 助手）。三个数答三个不同问题。 |
...
| **hooks/ 目录 vs scripts/ 目录** | 项目技能里：`scripts/` 放全部可执行脚本（含 5 个钩子脚本）；`hooks/` 只放 `hooks.json`（宿主钩子注册配置）。 |
| **draft vs active** | 项目技能的两个生命周期状态：draft=骨架未填满（禁全量门禁，防半成品错觉）；active=验证零占位符后激活（全量解锁）。 |
...
- **账本（ledger）**：`.swarm-yuan/` 下的 JSONL 留痕文件，四本：trace / decisions / gate-runs / gate-audit。
- **档位（profile）**：生成物的轻重配置：lite / standard / compliance / auto。
- **生成器（generator）**：swarm-yuan 本身，元技能——生成项目技能的技能。
- **项目技能（project skill / 生成物）**：生成器为某个项目产出的规则包（含 SKILL.md、references、scripts、rules.d、hooks 配置）。
- **门禁（gate）**：检查代码与仓库状态的 bash 函数，54 个，结果为 pass/fail/warn/skip 之一。
- **骨架（skeleton）**：生成器产出的未填充项目技能（draft 态），AI 填充后转 active。
- **骨架铁律（零占位符）**：draft 转 active 前必须无"待填充"占位符——防止半成品被当成品。
- **框架三件套**：一个技术框架的规则文档 + 门禁片段 + 双态夹具，三者一致性由 CI 核验。
- **hooks / 宿主 hooks**：AI 编码助手提供的时机钩子机制（写文件前/会话结束等），项目技能把检查脚本挂在上面。
