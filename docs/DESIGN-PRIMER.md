...
- **单一事实源**：所有"招牌数字"（55 门禁 / 74 框架 / 17 特征卡 / 178 变量 / 13 运行时整合……）只写在 `swarm-yuan/assets/facts.conf` 一处；`scripts/self-check.sh` 把这些数字与代码真值做**机械对账**（数函数、数文件、数数组元素），不符即失败——文档不允许手抄数字，"无手抄即无漂移"。
...
6. **门禁不抓逻辑错误**：55 个门禁全是词法/结构模式匹配（import 方向、注解模式、文件存在性、命令黑名单）——它们抓"不懂规矩"，不抓"想错逻辑"。逻辑错误的兜底是**测试**（check_test 跑测试命令）与**审查**（check_review），不是门禁本身。这是设计边界，不是缺陷（SKILL.md:16"机械脚本不假装能判断质量"）。
