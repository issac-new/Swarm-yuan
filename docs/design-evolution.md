...
**决策索引（R13 后）**：决策 1-17 见 §13 历史档案 A5 归档卷；决策 18-29 见本文前部；决策 30-32 自适应与压缩；决策 33-34 R13 重构与防复胖；决策 35 创造纪律（audit-claims-reality 轮）；决策 36 恰当应用（去教条化轮）；决策 37 R21 核心链条补强（复盘改进轮）；决策 38 审计收账轮（版本口径机器锚 + 认知面预算例外）；决策 39 Harness 实践吸收（证据分级 + 吸收边界 + 四新对象登记）；决策 40 双运行时纳入（存量来源补登记 + 机制级吸收不引依赖）；决策 41 知识生命周期吸收（四段协议显式化 + 过期三态 + 知识粒度裁决）。
