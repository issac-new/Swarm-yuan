...
| 场景 | 为什么适用 | 推荐 profile |
|------|-----------|-------------|
| **团队协作项目（≥2 人）** | 16 项特征卡让多人共享项目认知；36 门禁守护分支质量；门禁是机器执法不依赖人自觉 | standard |
| **中大型项目（≥80 文件或 ≥3 形态）** | 详尽构件库清单 + 调用链路分析让 AI 懂项目结构；框架规则引擎覆盖 61 框架 | standard |
...
| 场景 | 为什么不适用 | 替代方案 |
|------|------------|---------|
| **个人脚本/一次性原型** | 36 门禁 + 16 特征卡的认知负担远超脚本本身的复杂度；ROI 为负 | 直接用 AI 裸写，不套范式 |
...
swarm-yuan 确实重（20k 行文档 + 22k 行脚本 + 36 门禁 + 141 变量 + 61 框架规则），这是设计选择：

- **重量的来源**：16 项特征卡（认知 DNA）+ 36 门禁（执法）+ 61 框架规则（领域知识）+ 11 运行时整合（工具链）——每个子系统都有明确职责，非冗余堆砌。
