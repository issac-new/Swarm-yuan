...
|------|---------|----------|------|
| TOTAL 字节（代理指标，非模型 token） | 193226 | 156992 | 18.8% |
| TOTAL 行数 | 2144 | 1856 | 13.4% |

> ⚠️ **指标性质声明**：本表为字节/行数级代理，非模型 token 测量。LLM 分词器（BPE/SentencePiece 等）与字节无线性关系--同一段文本删减对 token 消耗的影响取决于词表命中，可能远高于或低于字节降幅。要获取真实 token 削减，须用目标模型 tokenizer 对 pre/post 目录计 token（`scripts/measure-tokens.sh` 待实现，见 P1-1 立项）。引用本表数字时须携带本声明，不得截断为"上下文削减 18.8%"。
