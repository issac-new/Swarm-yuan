...
# feat 分支只改动 src/ 并提交 → git diff --name-only main...HEAD 不命中只读目录
# 幂等：先清残留再重建，重建结果逐次一致
set -u
...
git checkout -q -b feat/scope-compliant
printf 'console.log("app v2");\n' > src/app.js
git add src/app.js
git -c user.name=fixture -c user.email=fixture@example.com -c commit.gpgsign=false commit -q -m "改动可写目录"
