#!/usr/bin/env bash
# test-capability-map-g25.sh — G25 能力地图断言族变异锁（R54 全面排查轮）
#
# 缺口背景：G25 六断言（载体/孤儿/幽灵/互指/分派零落档/随发或声明）是 warn 级自检，
...
#   态4 随发缺口：UNIVERSAL 块删一个已分派档 → 须 warn"随发缺口"
# 变异锁锚：函数经 sed 从 self-check.sh 提取（改名/移位即提取失败 → 红）。
#
...
mk_base
out=$(run_g25)
if printf '%s' "$out" | grep -q '能力地图双向对账一致' && ! printf '%s' "$out" | grep -q 'WARNHIT'; then
  ok "态0 正向：六断言全绿、零 warn"
else
  bad "态0 应全绿却出 warn：${out}"
fi
# 六面机器存在性：正向输出必须含六个 ✓ 标签（断言段被删即红）
for label in '存在且非空' '孤儿零容忍' '幽灵零容忍' '两级互指' '分派零落档' '随发或声明'; do
...
mk_base
printf '| ghost-fake-doc | 测试注入（R54 变异锁） | 流B | 触发 |\n' >> "$TMP/base/references/capability-map.md"
out=$(run_g25)
printf '%s' "$out" | grep -q 'WARNHIT|G25 幽灵档' \
  && ok "态2 幽灵档被拦（台账登记不存在档现形）" || bad "态2 幽灵注入未拦：${out}"
...
printf '%s' "$out" | grep -q 'WARNHIT|G25 分派落档' \
  && ok "态3 分派落档被拦（方法论不可从分派表现形）" || bad "态3 分派落档注入未拦：${out}"

# --- 态4 随发缺口：UNIVERSAL 块删一个已分发档（且 router 无【生成器侧】标注）→ 须 warn 随发缺口 ---
mk_base
