...
- violating 主触发 2 个 fail 意图：
  - fw_sharding_key_in_dml —— 对 SHARDED_TABLES 每表抽取 mapper XML 的 `<update>`/`<delete>` 块，引用该表但无对应分片键列名 → fail。
  - fw_sharding_broadcast_write —— 对 SHARDING_BROADCAST_TABLES 检索 `insert into`/`update`/`delete from` 命中 → fail...。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh sharding` 双态绿（violating 检出 / compliant PASS）。
