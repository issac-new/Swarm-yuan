# ios-swiftui fixture 说明

- violating 主触发 2 个 fail 意图：
  - fw_ios_webview_js —— WKWebView 且 javaScriptEnabled 未设 false → fail。
  - fw_ios_userdefaults_secret —— UserDefaults 含敏感 key 且无 Keychain → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh ios-swiftui` 双态绿（violating 检出 / compliant PASS）。
