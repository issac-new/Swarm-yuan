...
- violating 主触发 2 个 fail 意图：
  - fw_flutter_webview_unrestricted —— 剥注释后同文件命中 WebView/WebViewWidget/InAppWebView + `https?://` 且无导航拦截 API → fail。
  - fw_flutter_sharedprefs_secret —— `setString/setInt/...(` 调用点起 60 字符内含敏感 key 语义（大小写不敏感）→ fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh flutter` 双态绿（violating 检出 / compliant PASS）。
