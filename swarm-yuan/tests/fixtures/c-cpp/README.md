...
- violating 主触发 2 个 fail 意图：
  - fw_ccpp_unsafe_str —— strcpy/strcat/sprintf → fail。
  - fw_ccpp_gets —— gets( → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh c-cpp` 双态绿（violating 检出 / compliant PASS）。
