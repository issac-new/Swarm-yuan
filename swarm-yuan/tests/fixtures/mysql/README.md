...
- violating 主触发 2 个 fail 意图：
  - fw_mysql_charset —— DDL 检出 CHARSET=utf8/utf8mb3（非 utf8mb4）→ fail。
  - fw_mysql_deep_paging —— LIMIT/OFFSET 偏移量 ≥ 100000 → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh mysql` 双态绿（violating 检出 / compliant PASS）。
