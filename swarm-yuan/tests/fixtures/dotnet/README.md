...
- violating 主触发 2 个 fail 意图：
  - fw_dotnet_sql_injection —— ExecuteSqlRaw/CommandText 含拼接 → fail。
  - fw_dotnet_password_hash —— password 赋值无 Hash → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh dotnet` 双态绿（violating 检出 / compliant PASS）。
