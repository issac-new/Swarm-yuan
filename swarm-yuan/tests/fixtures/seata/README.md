...
- violating 主触发 2 个 fail 意图：
  - fw_seata_local_tx_mixed —— 同文件检出 @GlobalTransactional + @Transactional → fail (n/a)。
  - fw_seata_tcc_fence —— @TwoPhaseBusinessAction 无 useTCCFence=true → fail (n/a)。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh seata` 双态绿（violating 检出 / compliant PASS）。
