...
- violating 主触发 2 个 fail 意图：
  - fw_redis_no_expire —— 二参 set 无 TTL 且同文件无 expire/entryTtl → fail。
  - fw_redis_lock —— 裸 setnx/setIfAbsent 无 owner+过期/Redisson 痕迹 → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh redis` 双态绿（violating 检出 / compliant PASS）。
