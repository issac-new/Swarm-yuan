# elasticsearch fixture 说明

- violating 主触发 2 个 fail 意图：
  - fw_es_deep_pagination —— from≥10000（Java/JSON）→ fail；from≥1000 → warn 提示 search_after。
  - fw_es_wildcard_prefix —— wildcard/query_string 查询值以 * 开头 → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh elasticsearch` 双态绿（violating 检出 / compliant PASS）。
