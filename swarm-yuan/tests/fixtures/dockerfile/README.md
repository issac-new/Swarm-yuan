# dockerfile fixture 说明

- violating 主触发 3 个 fail 意图：
  - fw_dockerfile_latest_base —— FROM 命中 `:latest` 标签 → fail。
  - fw_dockerfile_root_user —— Dockerfile 无 USER 指令或显式 USER root/0 → fail。
  - fw_dockerfile_hardcoded_secret —— ENV/ARG 含密钥字面量（剔除 $ 引用）→ fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh dockerfile` 双态绿（violating 检出 / compliant PASS）。
