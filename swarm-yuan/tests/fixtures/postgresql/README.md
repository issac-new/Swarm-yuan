# postgresql fixture 说明

- violating 主触发 2 个 fail 意图：
  - fw_pgsql_pk_identity —— 检出 max(id)+1 / max(x_id)+1 应用层取号 → fail。
  - fw_pgsql_dml_where —— 单行 UPDATE/DELETE 无 WHERE → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh postgresql` 双态绿（violating 检出 / compliant PASS）。
