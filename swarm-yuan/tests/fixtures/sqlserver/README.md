# sqlserver fixture 说明

- violating 主触发 3 个 fail 意图：
  - fw_mssql_sql_injection —— EXEC(...+...) 拼接执行 / 拼接赋 SQL 串 → fail。
  - fw_mssql_sp_grant_public —— GRANT EXECUTE TO public → fail 越权。
  - fw_mssql_dml_nowhere —— 单行 UPDATE/DELETE 无 WHERE → fail。
...
- 验收：`bash swarm-yuan/tests/run-framework-fixture.sh sqlserver` 双态绿（violating 检出 / compliant PASS）。
