...
> 单一事实源。三层分离：**第一部分是设计内核（定稿，"神"）；第二部分是证据与规范（可查，"形"）；第三部分是施工档案（过程记录，非定稿）**。读者只需按角色取对应层，不必读全文。

| 层 | 部分 | 内容 | 给谁读 | 身份 |
|----|------|------|--------|------|
| **L1 设计内核** | §1-§6 | 理念→原则→架构→机制→使用→本体论 | 所有人（理解系统）/ 贡献者（改设计） | **定稿** |
| **L2 证据与规范** | §7-§10 | 决策索引、运行时基线、使用手册详情、术语与边界 | 查证者 / 使用者 / 升级者 | 规范·证据 |
| **L3 施工档案** | §11 | 决策史全文 + 历史档案 A1-A14（历次 WP/批次/轮次施工记录） | 溯源者 / 审计者 | **过程记录** |

**三条铁律（本文件自约束）**：① 设计内核只写"现在是什么"，不写"怎么改过来的"——过程归档案层；② 同一主题只有一处权威定义，其余处用指针（"详见 §X"）；③ 机器锚（`baseline_status=`/`FACT_*`/references 路由）所在段落逐字守恒，重构不动。

**读者地图**：5 分钟→§1 理念 + §5 使用；30 分钟→§1-§5 设计内核；改门禁→§4 机制 + §9 运行时基线 + §8 决策索引；接运行时→§9 + §13 A10/A11/A14；审计→§13 全卷。

# 第一部分 设计内核（定稿）

> 本部分只描述系统**现在是什么**，是全文唯一权威定义所在。演化过程见第三部分施工档案。
...
- **奠基期（ncwk，2026-07-02 ~ 07-19，决策 0 域）**：特征卡 9→12→13→14→16 项演化（12 项=用户首次明确"全部 12 项"；13/14=+认知基底/辩证范畴；15/16=+编排约束/详尽组件库清单——为修复"样本化填充 10% 覆盖"缺陷专门加入）；"左移"三节由用户一句话加入（2026-07-11）；三平台兼容硬约束由"脚本在 Windows 下不可用"触发（_resolve_path 可移植函数 + .bat 包装器全覆盖）；框架规则引擎方案 A 选型（内置框架库+门禁片段+生成时注入，2026-07-17，详见 §13 历史档案 A6（原 2026-07-17 框架规则引擎设计）；ncwk-dev 手写的 7 框架 28 项检查反向收割为片段库种子——从成功实践反哺范式库）；skill 上传四项规则（脱敏/路径泛化/去内部代号/推送前确认，首次发布确立）；运行时管理规则（research/ 仓库永远 checkout 最新 stable release tag，detached HEAD，永不 push）
- **决策 1-17**（早期，2026-07-20 后）：本文 §13 历史档案 A5 归档卷
- **决策 18-29**（自适应/诚实化/Palantir 期）：本文 §12 决策史前部
...
> 本部分是设计内核的支撑材料：决策的索引、外部依赖的真值表、操作级细节、术语与边界。可按需查阅，不必通读。
...
> 决策全文（含演化脉络）在 **§11 施工档案·决策史**。本表是索引：现行有效的设计决策按编号可查。决策 26/26.1/26.2（门禁负向预算）、27（升级整合纪律）、28/29/32/33/34/35 为影响现行架构的关键决策。

| 决策 | 主题 | 日期 | 全文 |
|------|------|------|------|
| 18 | 自适应轻重 + 质量优先偏置 + 授权无关化 | 2026-07-21 | §11 决策史 |
| 19 | catchphrase 单一事实源 facts.conf | 2026-07-21 | §11 决策史 |
| 20 | 任务类型维度实装 | 2026-07-21 | §11 决策史 |
| 21 | WP-P5 enforce_level 随 profile 调整——延后到  | 2026-07-21 | §11 决策史 |
| 22 | profile 动态升档——detect-profile-drift 只升不 | 2026-07-21 | §11 决策史 |
| 23 | spec 三级机器执法——detect-spec-scale 从 spec  | 2026-07-21 | §11 决策史 |
| 24 | 技术栈复杂度反作用 profile——形态/框架/微服务信号升档 | 2026-07-21 | §11 决策史 |
| 25 | 范式定位声明——适用/不适用场景显式化 | 2026-07-21 | §11 决策史 |
| 30 | auto_detect_profile 偏置方向修正——只升不降→信号明确才 | 2026-07-21 | §11 决策史 |
| 31 | 门禁分层 enforce_level—— 2026-07-21 WP-Q1 | 2026-07-21 | §11 决策史 |
| 26 | 复杂度负向预算--门禁/变量数冻结上限 | 2026-07-26 | §11 决策史 |
| 27 | 运行时升级整合纪律--吸收优先于新增门禁 | 2026-07-26 | §11 决策史 |
| 28 | 标记沿调用链传播 | 2026-07-31 | §11 决策史 |
| 29 | 语义/动能二分显式命名 + 动作授权锚定组件标记 + FDE 反向传播形式化 | 2026-07-31 | §11 决策史 |
| 32 | 上下文窗口自适应压缩——SKILL.md 分层折叠 + frontmatte | 2026-08-01 | §11 决策史 |
| 33 | 范式作为条件而非内容——去抽象化重构与落地优先原则 | 2026-08-21 | §11 决策史 |
| 34 | 概念落地问责 + 吸收三问 + 生成物税制 | 2026-08-21 | §11 决策史 |
| 35 | 创造纪律——未创建且未运行的机制不得声称 | 2026-08-24 | §11 决策史 |
| 33-35 | 范式条件化/防复胖三制度/创造纪律 | 2026-08 | §11 决策史 |
...
2. **版本漂移（1 项 drifted，2026-09-01 R16 重核后）**：comet 0.3.9→0.4.0-rc.1（仍观望——20 beta + 1 rc 正式版未出；R16 裁决：rc 阶段仍落地 117k 行新子系统，等正式版一次做对）。R16（2026-09-01）全量重核把 gsd-core/claude-mem/ocr/graphify/gstack/ruflo/ECC/codex-security + claude-code/codex-cli/dsh 11 项升级到最新并吸收对应方法论（详见各条目吸收注记 + §13 历史档案 A14 运行时升级报告）。
...
> 本部分是**演化过程的原始记录**：决策史全文与历次工作包（WP）/批次/轮次的施工档案 A1-A14。它们回答"系统是怎么变成今天这样的"，但**不构成对现状的权威描述**——现状以第一部分为准。审计与溯源用。
