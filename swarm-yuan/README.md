...
> 本 README 是唯一文档——docs/ 下全部内容已整合于此（当前事实源 + 历史档案）。数字真值仍以 `assets/facts.conf` 为准。
...
#### design-philosophy-consistency.md
