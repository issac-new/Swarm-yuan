...
**特征卡驱动一切：** → 文件填充（SKILL.md 铁律 ← 第 2/6 项、codebase.md ← 第 4 项、reference-manual.md 组件库 ← 第 11 项）→ 门禁配置（precheck.conf 156 个变量从特征卡推导）→ 开发流程（开始新需求时从第 11 项检索可复用单元）。
...
| 维度 | 数值 |
|------|------|
| **特征卡** | **16 项（驱动全部文件 + 156 个门禁变量 + 开发流程）** |
