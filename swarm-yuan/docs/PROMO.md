...
17 项特征卡定义「项目应该是什么样的」，48 个门禁验证「代码是否符合」。两者构成闭环——特征卡驱动门禁配置，门禁验证特征卡定义的规则。**门禁还按执法强度分三档（决策 19）**：strict 17（真 fail 阻断）/ warn 20（混合）/ advisory 10（永不 fail，观测类）——让"门禁是执法"不再是一句宣称，而是机器可核验的分层。
...
**→ 门禁配置：** precheck.conf 156 个变量从特征卡推导——WRITABLE_DIRS ← 第 2 项、TEST_CMD ← 第 5 项、LAYER_DEFS ← 第 3 项、STABLE_GLOBS ← 第 11 项、SERVICE_DIRS ← 第 10 项……
