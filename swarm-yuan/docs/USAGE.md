...
AI 写代码前必须先认识项目——概念→结构→空间→映射→规律→处理。不认识就写 = 盲动。swarm-yuan 用 **16 项特征卡** 完成认知，用 **45 个质量门禁** 守护行动。
...
**特征卡 → 门禁配置（Step 5）：** precheck.conf 的 151 个变量从特征卡推导：
...
**特征卡 → 开发流程（日常使用）：** 开始新需求时，AI 从特征卡第 11 项检索可复用单元，预填 spec §5.5 复用约束。编码时 AI 查特征卡第 11 项的组件库清单，拼装优先。提交前 45 个门禁按特征卡配置的规则检查。
...
特征卡定义了「项目应该是什么样的」，45 个门禁验证「代码是否符合特征卡定义的规则」。

### 门禁分层（决策 19，横切维度）

45 门禁按 `fail()` 调用能力分三档（strict 14 / warn 20 / advisory 10），与 core/standard/compliance 三档正交（一个门禁同时属于 core + strict，或 standard + advisory）：

| 分层 | 数量 | fail() 能力 | 行为 | 门禁清单 |
|------|------|------------|------|----------|
| **strict** | 14 | ≥3 fail | 真 fail 阻断交付 | branch / layer / reuse / security / shift-left / compliance / sbom / privacy / authz / requirements / rtm / release-sign / dengbao / pia |
| **warn** | 20 | 1-2 fail | 能 fail 但触发窄，混合 warn | scope / build / test / sensitive / review / stable-diff / deps / adr / contract / impact / service / api / frontend / domain / knowledge / docs-pack / crypto / framework / sast-deep / oss-eval |
| **advisory** | 10 | 0 fail | 永不 fail，只 warn/pass（认知/观测类） | consistency / link-depth / consistency-cross / state / cognition / mermaid / operate / decision-audit / canary / cwe-audit |
...
### 合规门禁（13 个，独立 `--compliance-suite` 按需执行，未配置则静默跳过）
...
| `compliance` | 金融/医疗等强监管交付 | standard + 标准合规矩阵参考（`references/standards-compliance.md`，配套 `--compliance-suite` 合规 13 门禁） |
...
| 标准 | §1-§13 + §5.5/§5.6/§5.7 约束段 | `--all-full`（标准 27） | 新功能 / 改接口 |
...
bash .claude/skills/my-project-dev/scripts/precheck.sh --compliance-suite  # 合规 13 门禁（强监管交付按需）
```
...
| 维度 | 数值 |
|------|------|
| **特征卡** | **16 项（驱动全部文件 + 151 个门禁变量 + 开发流程）** |
| **质量门禁** | **44 个（核心 10 + 架构 17 + 合规 13，特征卡立法 + 门禁执法）** |
| 运行时工具 | 11 |
...
bash scripts/precheck.sh --compliance-suite  # 合规 13 门禁（强监管交付按需）
```
