...
AI 写代码前必须先认识项目——概念→结构→空间→映射→规律→处理。不认识就写 = 盲动。swarm-yuan 用 **17 项特征卡** 完成认知，用 **48 个质量门禁** 守护行动。
...
**特征卡 → 门禁配置（Step 5）：** precheck.conf 的 158 个变量从特征卡推导：
...
**特征卡 → 开发流程（日常使用）：** 开始新需求时，AI 从特征卡第 11 项检索可复用单元，预填 spec §5.5 复用约束。编码时 AI 查特征卡第 11 项的组件库清单，拼装优先。提交前 48 个门禁按特征卡配置的规则检查。
...
特征卡定义了「项目应该是什么样的」，48 个门禁验证「代码是否符合特征卡定义的规则」。

### 门禁分层（决策 19，横切维度）

48 门禁按 `fail()` 调用能力分三档（strict 17 / warn 21 / advisory 10），与 core/standard/compliance 三档正交（一个门禁同时属于 core + strict，或 standard + advisory）：
...
| 5 | AI 配置 precheck.conf——**158 个变量从特征卡推导** |
