...
_fw_lombok_check() {
  ...
  # 代码正文过滤辅助：剔除单行注释 // 与 javadoc 块注释 * 行，避免 grep 误命中 javadoc 文本中的注解名
  # 用法：_fw_code_only <file> → stdout 输出该文件去注释后的代码行（保留 package/import/class/方法体/注解使用行）
  _fw_code_only() {
  ...
  if [[ ${#srcarr[@]} -eq 0 ]]; then
    ...
    for efile in "${srcarr[@]}"; do
      ...
      if _fw_code_only "$efile" | grep -qE '@Entity\b'; then
...
"
    ...
    if [[ -z "$entity_files" ]]; then
      ...
      while IFS= read -r efile; do
        [[ -z "$efile" ]] && continue
        if _fw_code_only "$efile" | grep -qE '@Data\b'; then
          data_violations="${data_violations}${efile}
"
        fi
      done <<< "$entity_files"
      if [[ -n "$data_violations" ]]; then
        fail "fw_lombok_data_jpa: @Entity 上同时标 @Data（懒加载字段触发 LazyInitializationException/N+1，双向关联 StackOverflow），改用 @Getter @Setter + 字段级 @ToString.Exclude/@EqualsAndHashCode.Exclude:
${data_violations}"
      else
        pass "fw_lombok_data_jpa: @Entity 类均未用 @Data"
      fi
    fi
  fi
  ...
  if [[ ${#srcarr[@]} -eq 0 ]]; then
    ...
    for sfile in "${srcarr[@]}"; do
      if _fw_code_only "$sfile" | grep -qE '@Slf4j\b'; then
...
"
    ...
    if [[ -z "$slf4j_files" ]]; then
      ...
      while IFS= read -r sfile; do
        [[ -z "$sfile" ]] && continue
        if _fw_code_only "$sfile" | grep -qE 'LoggerFactory\.getLogger'; then
          slf4j_dup="${slf4j_dup}${sfile}
"
        fi
      done <<< "$slf4j_files"
      if [[ -n "$slf4j_dup" ]]; then
        fail "fw_lombok_slf4j_dup: @Slf4j 已生成 log 字段，同文件又手写 LoggerFactory.getLogger（字段重复声明或双 Logger 实例）:
${slf4j_dup}"
      else
        pass "fw_lombok_slf4j_dup: @Slf4j 类未手写 LoggerFactory.getLogger"
      fi
    fi
  fi
  ...
  if [[ ${#srcarr[@]} -eq 0 ]]; then
    ...
    for bfile in "${srcarr[@]}"; do
      if _fw_code_only "$bfile" | grep -qE '@Builder\b|@SuperBuilder\b'; then
...
"
      ...
      if [[ -n "$bwarn" ]]; then
        warn "fw_lombok_builder_jackson: @Builder 用于 Jackson 反序列化须配 @Jacksonized 或 @NoArgsConstructor+@AllArgsConstructor（裸 @Builder 仅生成 package-private 全参构造，Jackson 反序列化 InvalidDefinitionException）:
${bwarn}"
      else
        pass "fw_lombok_builder_jackson: @Builder 均配 Jackson 协议"
      fi
    fi
  fi
  ...
  if [[ ${#srcarr[@]} -eq 0 ]]; then
    ...
    for cfile in "${srcarr[@]}"; do
      if _fw_code_only "$cfile" | grep -qE '@RequiredArgsConstructor\b|@AllArgsConstructor\b'; then
...
"
    ...
    if [[ -z "$ctor_files" ]]; then
      ...
      for cfile in "${cfa[@]}"; do
        ...
        my_cls=$(_fw_code_only "$cfile" | grep -E '^\s*(public\s+)?(final\s+|abstract\s+)*class\s+\w+' | head -1 | sed -E 's/.*class[[:space:]]+([A-Za-z_][A-Za-z0-9_]*).*/\1/')
        [[ -z "$my_cls" ]] && continue
        # 找本文件引用的其他 ctor 文件类作为 final 字段类型
        for other_file in "${cfa[@]}"; do
          [[ "$other_file" == "$cfile" ]] && continue
          other_cls=$(_fw_code_only "$other_file" | grep -E '^\s*(public\s+)?(final\s+|abstract\s+)*class\s+\w+' | head -1 | sed -E 's/.*class[[:space:]]+([A-Za-z_][A-Za-z0-9_]*).*/\1/')
          [[ -z "$other_cls" ]] && continue
          # 本文件含 "private final <other_cls>" 且对方文件含 "private final <my_cls>" → 互引
          if _fw_code_only "$cfile" | grep -qE "private[[:space:]]+final[[:space:]]+${other_cls}\b" \
             && _fw_code_only "$other_file" | grep -qE "private[[:space:]]+final[[:space:]]+${my_cls}\b"; then
...
"
      ...
      if [[ -n "$circ_warn" ]]; then
        warn "fw_lombok_requiredargs_circular: 检出两类互引 final 字段 + 均带 @RequiredArgsConstructor/@AllArgsConstructor（Spring 6 默认禁循环依赖，启动期 BeanCurrentlyInCreationException）:
${circ_warn}"
      else
        pass "fw_lombok_requiredargs_circular: 未检出明显的构造注入互引循环"
      fi
    fi
  fi
...
"
    ...
    if [[ -n "$eoh_bad" ]]; then
      warn "fw_lombok_equals_callsuper: @EqualsAndHashCode 未声明 callSuper= 且类继承非 Object（子类 equals 漏父类字段，须显式 callSuper=true/false 消除 warning）:
${eoh_bad}"
    else
      pass "fw_lombok_equals_callsuper: @EqualsAndHashCode 均显式声明 callSuper 或无 extends"
    fi
  fi
...
"
    ...
    if [[ -n "$lazy_bad" ]]; then
      warn "fw_lombok_equals_lazy: @EqualsAndHashCode 未排除 JPA 关联字段（懒加载字段进 equals/hashCode 触发 LazyInitializationException，须 exclude=/of= 或 @EqualsAndHashCode.Exclude）:
${lazy_bad}"
    else
      pass "fw_lombok_equals_lazy: @EqualsAndHashCode 已排除关联字段或无 JPA 关联"
    fi
  fi
...
"
    ...
    if [[ -n "$st_bad" ]]; then
      warn "fw_lombok_sneaky_throws: @SneakyThrows 出现在 Service/Controller/Facade/Api 类（隐藏受检异常破坏契约，调用方无法 catch，须 throws 声明或 wrap 业务异常）:
${st_bad}"
    else
      pass "fw_lombok_sneaky_throws: @SneakyThrows 仅在 util/lambda 等狭窄场景"
    fi
  fi
...
"
    ...
    if [[ -n "$cu_hits" ]]; then
      warn "fw_lombok_cleanup: 检出 @Cleanup（JDK 7+ 优先 try-with-resources：@Cleanup 清理方法抛异常会掩盖原异常，多资源嵌套关闭顺序敏感）:
${cu_hits}"
    else
      pass "fw_lombok_cleanup: 无 @Cleanup 用法"
    fi
  fi
  ...
  if [[ ${#srcarr[@]} -eq 0 ]]; then
    ...
    for vfile in "${srcarr[@]}"; do
      local code
      code=$(_fw_code_only "$vfile")
      # import lombok.val/var （import 行也算代码行，未被 _fw_code_only 过滤）
...
"
    ...
    if [[ -n "$gl_hits" ]]; then
      warn "fw_lombok_getter_lazy: 检出 @Getter(lazy=true)（字段类型被改写为 AtomicReference，禁止直接访问字段；几乎总被访问的场景直接初始化更省；不可变对象才用 cacheStrategy）:
${gl_hits}"
    else
      pass "fw_lombok_getter_lazy: 无 @Getter(lazy=true) 用法"
    fi
  fi
...
"
    ...
    if [[ -n "$nn_bad" ]]; then
      warn "fw_lombok_nonnull_validation: @Valid DTO 标 lombok @NonNull 但缺 jakarta.validation @NotNull（@NonNull 仅方法入口 fail-fast，不接入 Bean Validation pipeline，请求体 null 字段会进 Service 抛 NPE 而非 400）:
${nn_bad}"
    else
      pass "fw_lombok_nonnull_validation: @Valid DTO 的 @NonNull 字段均已配 jakarta @NotNull 或无 lombok @NonNull"
    fi
  fi
