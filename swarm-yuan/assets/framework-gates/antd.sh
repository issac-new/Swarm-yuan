...
"
  ...
  if [[ -n "$static_bad" ]]; then
    fail "fw_antd_app_useapp: 检出 message/notification/modal 静态调用（React 18+ 须用 App.useApp，静态调用无法消费 ConfigProvider context，主题/locale 失效）:
${static_bad}"
  else
    pass "fw_antd_app_useapp: 未检出静态 message/notification/modal（已用 useApp）"
  fi
...
"
  ...
  if [[ -n "$full_imp" ]]; then
    warn "fw_antd_on_demand_import: 检出全量 import antd（须用 unplugin 按需引入，否则包体过大）:
${full_imp}"
  else
    pass "fw_antd_on_demand_import: 未检出全量 import（已按需引入或未使用）"
  fi
...
"
  ...
  if [[ -n "$fc_bad" ]]; then
    warn "fw_antd_form_useform: 检出 Form.create 废弃 API（4.x 起移除，须用 useForm Hook）:
${fc_bad}"
  else
    pass "fw_antd_form_useform: 未检出 Form.create（已用 useForm 或无表单）"
  fi
...
"
  ...
  if [[ -n "$tbl_bad" ]]; then
    warn "fw_antd_table_virtual: Table 绑定大数据源未配 virtual/scroll（>1k 行须虚拟滚动）:
${tbl_bad}"
  else
    pass "fw_antd_table_virtual: Table 已配虚拟滚动或数据量小（或无表格）"
  fi
...
"
  ...
  if [[ -n "$th_bad" ]]; then
    warn "fw_antd_configprovider_theme: 直接覆写 .ant-* 类样式（须用 ConfigProvider theme.token，升级即失效）:
${th_bad}"
  else
    pass "fw_antd_configprovider_theme: 未检出直接覆写 .ant-* 类（已用 ConfigProvider token）"
  fi
...
"
  ...
  if [[ -n "$fi_bad" ]]; then
    warn "fw_antd_form_item_name: Form.Item 未配 name（须与 data 字段对应，否则校验/取值失效）:
${fi_bad}"
  else
    pass "fw_antd_form_item_name: Form.Item 均配 name（或无表单项）"
  fi
...
"
  ...
  if [[ -n "$mdl_bad" ]]; then
    warn "fw_antd_modal_destroyonclose: Modal 未配 destroyOnClose（关闭后子组件状态残留）:
${mdl_bad}"
  else
    pass "fw_antd_modal_destroyonclose: Modal 均配 destroyOnClose（或无 Modal）"
  fi
...
"
  ...
  if [[ -n "$sel_bad" ]]; then
    warn "fw_antd_select_remote: Select showSearch 未配 onSearch 远程搜索（大数据选项须远程，否则全量渲染卡顿）:
${sel_bad}"
  else
    pass "fw_antd_select_remote: Select 远程搜索配置合理（或无 showSearch）"
  fi
...
"
  ...
  if [[ -n "$up_bad" ]]; then
    fail "fw_antd_upload_size_limit: Upload 未配 beforeUpload 大小校验（无限制可上传超大文件致存储耗尽/DoS）:
${up_bad}"
  else
    pass "fw_antd_upload_size_limit: Upload 均配大小限制（或无上传）"
  fi
...
"
  ...
  if [[ -n "$ell_bad" ]]; then
    warn "fw_antd_typography_ellipsis: 检出手写 .slice 截断显示（须用 Typography.Paragraph/Ellipsis ellipsis，自适应+tooltip）:
${ell_bad}"
  else
    pass "fw_antd_typography_ellipsis: 未检出手写截断（已用 Typography.Ellipsis 或无截断）"
  fi
...
"
  ...
  if [[ -n "$grid_bad" ]]; then
    warn "fw_antd_grid_responsive: Col 仅用静态 span 无响应式断点（须 xs/sm/md/lg 配置，否则移动端错位）:
${grid_bad}"
  else
    pass "fw_antd_grid_responsive: Col 响应式配置合理（或无 Grid）"
  fi
}
