@echo off
REM env-setup.bat - Windows 包装器，自动查找 bash 并运行 env-setup.sh
REM 用法:
REM   env-setup.bat    检测开发环境 + 外部资源连通性 + 工具权限
...
if "!BASH_KIND!"=="wsl" (
    wsl bash "!BASH_DIR!/env-setup.sh" %*
) else (
    "!BASH_CMD!" "!BASH_DIR!/env-setup.sh" %*
)
