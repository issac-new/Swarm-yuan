...
for _conf_var in MYBATIS_MAPPER_DIRS SQL_INJECTION_WHITELIST SHARDING_KEY_COLUMNS \
    ...
    AUTHZ_SCAN_DIRS AUTHZ_EXTRA_PATTERNS CRYPTO_SCAN_DIRS \
...
ALL_GATES_COMPLIANCE=(check_compliance check_docs_pack check_sbom check_privacy check_authz check_requirements check_crypto check_rtm check_dengbao check_pia check_release_sign)
...
GATE_FLAGS=(--branch --scope --build --test --sensitive --consistency --review --reuse --deps --security --layer --stable-diff --link-depth --adr --contract --consistency-cross --impact --service --api --state --frontend --cognition --domain --knowledge --mermaid --shift-left --framework --compliance --docs-pack --sbom --privacy --authz --requirements --crypto --rtm --dengbao --pia --release-sign)
...
if [[ -z "${_gate_fn:-x}" ]]; then
  ...
  check_authz; check_requirements; check_crypto; check_rtm; check_dengbao; check_pia; check_release_sign
fi
