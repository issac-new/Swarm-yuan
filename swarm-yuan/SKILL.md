...
swarm-yuan 的 31 个门禁服务于一条认知递进链。核心理念：**呈现递进的关系，而非仅关注计算**。
...
- 门禁 precheck.conf 162 变量：AI 从特征卡**主动推导建议配置**，用户评估后确认
...
10. **AI 运行门禁**：`precheck.sh --all`（核心 10）→ fail 自动修复重跑 → `--all-full`（全 31：核心 10+架构 17+合规 4）
...
| check | `scripts/precheck.sh` | 31 个门禁子命令（核心 10 + 架构 17 + 合规 4：`--compliance` 标准合规矩阵核验 / `--docs-pack` 文档包清单 / `--sbom` SBOM 生成+许可证块名单 / `--privacy` 个人信息扫描，随 `--all-full` 执行，未配置静默跳过；另含 `--shift-left` 左移：测试设计/变更影响/可观测性） + **框架门禁片段注入区**（`# >>> swarm-yuan:framework-gates >>>` ... `# <<< swarm-yuan:framework-gates <<<` 标记区块，由 `--inject-frameworks` 写入） |
...
| 标准合规矩阵（GB/T 25000.51/8566/8567/9386 + 安全标准 × 31 门禁映射 + 豁免登记） | `references/standards-compliance.md` |
