...
- 稳定性标注：见 §11f（唯一定义点：稳定层/不稳定层/禁止改层）
  ...
  → 每个稳定单元记录：签名、路径、用途、复用方式（五维字段见 README.md 完整文档/历史档案/FIVE_DIMENSIONS，供 dev-guide.md 引用）
...
> **本节是"稳定单元"概念的唯一权威定义点**——全仓其余文件引用此概念时一律指回"见 references/exploration-guide.md §11f"，不重复定义。每个稳定单元记录五维字段（签名/路径/用途/复用方式/稳定性标注），五维字段详解见 README.md 完整文档/历史档案/FIVE_DIMENSIONS。
