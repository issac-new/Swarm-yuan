...
- **R24（2026-09-11）**：ocr **v1.11.8**——**Rego policy review 支持**（规则面新语言）+ **预览选择与执行对齐**（preview selection aligns with execution——展示与执行口径一致族新样本，与 gsd「no-op 报真实条件」同族）。档案 `docs/research/R24-runtime-refresh.md`。
- **R26（2026-09-12）**：ocr **v1.11.9**——**`--preview` 应用与评审相同的选择集**（#801——预览=执行口径族**第三实例**：预览路径必须消费与执行路径同一选择函数，不允许平行实现）。viewer session compare 页（#1175）纯查看器面不吸收。档案 `docs/research/R26-runtime-refresh.md`。
- **R27（2026-09-13）**：ocr **v1.12.0**——名义 minor 实薄轮：分组返回文件索引（#1209 perf 实现面）+ viewer store 会话态（查看器面）+ glm-5.3 通道命名 chore，无新方法论原语，对账通过。**口径注记：版本号标签 ≠ 实质判定**（吸收判定以内容为准，不以 semver 标签）。档案 `docs/research/R27-runtime-refresh.md`。
- **R29（2026-09-16）**：gstack **v1.87.0.0**（71f6048→4a3c6a8 两步）——①**v1.86.0.0 按 harness 路由外部评审**（#2850）：外部评审意见按来源 harness 的能力/语义路由到对应处理通道——评审输入不是同质文本流，按来源结构化消费是前提（「模型生成文本不是协议面」的评审侧镜像）；②**v1.87.0.0 CSO 可验证审计 + 可重放修复包**（#2852）：verified audits = 审计结论附可独立复核的证据链；replayable repair bundles = 修复动作打包为可从快照重放的 bundle（运行时组装安全金丝雀 + release proof 边界 + 完整评估报告前置 + 过期快照须从供给源重放）——把「审过了/修好了」从断言变为可验证对象（诚实口径族治理面极端形态）。ocr **v1.12.2** 薄轮对账通过（preview 总量圈定 tracked 变更集——展示=执行口径族延续）。档案 `docs/research/R29-runtime-refresh.md`。
- **R32（2026-09-16）**：ocr **v1.12.4**（R31 扫描后数小时出线，薄轮）——①#1310 diff 对超大未跟踪文件**读前跳过**：进入读取前先量体积边界，与「预算前置」族同向（DeadlineBudget/调用方剩余死线封顶的静态面兄弟：不可控输入先验界再消费）；②#1288 workspace 模式 untracked binary 判 binary：不可分析输入显式分类，不把二进制当文本硬读（诚实族）；③#1156 action 可选 pr_number 输入 + workflow_run 触发回退：事件源不可靠时的第二触发通道。方法论注记级吸收，无门禁增量。档案 `docs/research/R32-runtime-refresh.md`。
