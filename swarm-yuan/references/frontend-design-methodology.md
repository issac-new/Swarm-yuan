...
swarm-yuan 当前 17 项特征卡 / 54 门禁 / 74 框架规则集覆盖前端的「机械维度」（组件枚举/调用链/状态管理/props/循环依赖/CSS 污染/框架代码模式），但**零视觉设计规则**——不回答「这个 UI 是否在视觉上合格」。
...
## R4 吸收（2026-08-21）：impeccable v4.1 对抗性 verdict + 证据完整性

### 对抗性 verdict（v4.1.0）

impeccable v4.1.0 设计评审工作流升级：**每个生成的设计方向带对抗性 verdict**——多方案先内部比较（哪个在什么维度更优）再呈交用户，而非把 N 个方案原样丢给用户选。
-  safer/bolder 沿"熟悉-大胆"轴定向重掷（用户对某方案说"再大胆一点"时，沿该轴重新生成而非随机换方向）
-  IMPECCABLE'S PICK 展示模型自选（带理由）

本仓对齐：`decisions.jsonl` 的 `alternatives` 字段已对齐（WP-R12-D：决策须列备选）；可借鉴"每个备选带对抗性 verdict"——不只列"还有 B/C"，而是"B 在 X 维度更优但 Y 维度更差"。
- 审查口径：trace-log.sh --decision 的 --alternatives 字段若只列名字不给对比 → 判 Important（要求补对抗性 verdict）

### 证据完整性：证据坏则重取证重跑（v4.1.1）

impeccable v4.1.1 落地"证据完整性"纪律：
- 截图坏/缺时**重拍并重跑评审**——不基于坏证据下结论
- 用户截图反驳触发**全量复审**（不是局部补丁）
- 拒绝假抠图（圆形 mask 冒充 cutout）

本仓对齐：`inventory-verify.sh --path-check` 的 HALLUCINATION 阻断同构（清单登记的路径必须在仓库真实存在，幻觉路径阻断 mark-active）。
- 审查口径：任何"基于证据的判断"（review/审计/测试）——证据本身不完整/不可信时，结论降级为 warn 并重取证，不在坏证据上硬下结论
