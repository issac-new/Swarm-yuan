...
swarm-yuan 当前 17 项特征卡 / 54 门禁 / 74 框架规则集回答了「项目应该是什么样的」与「代码是否合规」，但**没有显式回答「我新加的这条规则，应该放进哪一层上下文」**——是写进 SKILL.md 常驻、还是 precheck.conf 配置、还是 reference 按需读、还是门禁机器执法、还是 hooks 阻断。
