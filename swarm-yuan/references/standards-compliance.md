...
> 证据基线：`docs/research/R7-quality-standards.md`、`docs/research/R8-security-standards.md`（2026-07-20，条款号均出自该两报告，禁止虚构）；门禁语义基线：`swarm-yuan/assets/precheck.sh`（31 门禁 = 27 既有 + 4 合规新增）与 `swarm-yuan/assets/precheck.conf`。
...
2. **配置入口**：`precheck.conf` 标准合规段 16 变量（`COMPLIANCE_MATRIX_FILE` / `DOCS_PACK_PROFILE` / `SBOM_REQUIRED` / `PRIVACY_SCAN_DIRS` 等）驱动 §C/§D/§E 对应的 `--docs-pack` / `--sbom` / `--privacy` 门禁。
...
依据 GB/T 25000.51-2016（RUSP）§5.2 用户文档集要求、§5.3 软件质量要求（八特性，与 GB/T 25000.10-2016 质量模型一致），将 31 个门禁按八特性逐行登记；「测量函数/阈值/证据」列对齐 GB/T 25000.21-2019 测度元素格式（R7 Q-06，本文件即 P0 级「门禁级四元组登记」的落地）。特性名同时标注 ISO/IEC 25010:2023 双轨命名（R7 ⑦：国标尚未跟进 2023 版）。
...
| 功能适合性 | `--framework`（check_framework） | 57 规则集 `_fw_<id>_<rule>` 逐条判定计数 | 任一规则 fail 即 fail；`ACTIVE_FRAMEWORKS` 空 → 静默跳过 | 各框架规则 fail 行（稳定 id）+ 57/57 fixture 绿 |

差距登记：需求↔测试追溯（RTM）无门禁 —— 缺口（P2，R7 Q-11 / 差距矩阵 §1.1 功能适合性行）。
...
| ⑤ AI 配置 precheck.conf | 6.3 技术管理过程组 | 质量保证策划 + 配置管理（测度元素实例化） | precheck.conf（162 变量真实值） |
...
- **ISO/IEC/IEEE 29148:2018**：无占位标记/唯一 ID/可验证/RTM ↔ 零占位符机器执法（P0，`generate-skill.sh --verify-completeness`）+ 需求 lint（P1-9）+ RTM（P2，Q-11）。
...
| 标准条款 | 要求 | 门禁映射 | 状态 |
|---|---|---|---|
| GB/T 39786-2021 8.4（三级应用和数据安全：身份鉴别/重要数据机密性/完整性/不可否认性） | SM2/3/4 国密算法保护 | 无 | ❌ 缺口（P2 国密门禁：算法白名单 lint）；P0 矩阵登记（本行） |
...
| 标准要求 | 门禁映射 | 状态 |
|---|---|---|
| 可靠性/安全性/性能/可维护性四特性弱点静态计数 | `--security` 10 模式族 + framework 57 规则集（词法层 grep） | 🟡 部分：**无 CWE 对齐**（R2§8）→ 缺口（P1 元数据：676 子门禁 CWE 元数据分级） |
...
| PS（保护软件 PS.1–PS.3：代码防篡改/**发布完整性验证机制**/发布归档） | PS.1/PS.3 部分 ↔ git 工作流 + `--stable-diff`；**PS.2 发布签名/provenance** | 无 | 🟡 部分；PS.2 ❌ 缺口（P2 发布签名门禁） |
| PW（生产安全软件 PW.1–PW.9：安全设计/编码/构建/评审/测试/默认安全配置） | 门禁体系主体：`--security`/`--layer`/`--review`/`--test`/`--shift-left` | 31 门禁 | ✅ 主体覆盖（PW≈门禁体系，R8 §⑧） |
...
| ASVS V6–V10 认证/会话/授权；CWE-862/863/284/639 授权类四弱点（2025 榜） | 无服务端授权检查门禁 | ❌ 缺口（P1 授权类门禁，R8 #20） |
...
| SLSA v1.0 Build L2 / Sigstore（签名 provenance） | 构建来源证明可验签 | 无 | ❌ 缺口（P2，cosign+in-toto） |
