...
| ⑤ AI 配置 precheck.conf | 6.3 技术管理过程组 | 质量保证策划 + 配置管理（测度元素实例化） | precheck.conf（142 变量真实值） |
...
| 标准要求 | 门禁映射 | 状态 |
|---|---|---|
| 可靠性/安全性/性能/可维护性四特性弱点静态计数 | `--security` 10 模式族 + framework 62 规则集（词法层 grep） | 🟡 部分：高频弱点 CWE 轻量映射已建（`security-spec.md` §5.1 CWE 元数据分级参考表，15 项高频弱点 ↔ CWE ↔ OWASP ↔ 标准依据）；完整 676 子门禁 CWE 元数据分级仍 P1 缺口（R2§8） |
