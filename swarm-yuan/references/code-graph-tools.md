...
- 版本注记：v0.9.65（2026-09-21 R41 核）——**JS let/const 块级作用域绑定**：绑定按所在块（block）解析而非函数级，不同块的 `let x` 是不同绑定实体——「**同名 ≠ 同物**」判别原语再添实例（与 R34 god node guard 同族），配块级遮蔽回归测试；**fail-closed 保全节点跨遍存活**（#3695：显式判为 fail-closed preserved 的节点不得被后续 AST 所有权逐出遍删除——保全承诺必须在所有后续遍保持，诚实族图谱侧）+ Go interface/Swift protocol 方法需求提取（类型约束面进图谱 = 实现关系边提取面拓宽）+ Java enum 成员归属 enum 而非文件（归属精确性）+ Verilog 实例链接本地定义 + vis-network 导出栈溢出崩溃修复（查看器稳定性）+ Pillow CVE floor（cve-2026-54058→12.3.0，供应链安全面）+ 删两条永不触发的 symlink 假测试（测试卫生族：永不触发的测试 = 假绿）。**v1.0.0 异源裁决维持不取。**档案 `docs/research/R41-runtime-refresh.md`。
