...
| 门禁 id | 级别 | 实现逻辑 | 依赖变量 |
|---------|------|---------|---------|
| fw_nest_validation_whitelist | fail | 无 ValidationPipe → fail；无 whitelist: true → fail；缺 forbidNonWhitelisted → warn | NEST_SRC_GLOBS |
...
| fw_nest_exception_filter | warn | 无 @Catch/useGlobalFilters/APP_FILTER → warn | NEST_SRC_GLOBS |
| fw_nest_serialization | warn | 无 ClassSerializerInterceptor/@Exclude → warn | NEST_SRC_GLOBS |
| fw_nest_typeorm_sync | fail | synchronize: true → fail 生产数据风险 | NEST_SRC_GLOBS |
| fw_nest_swagger | warn | 无 @nestjs/swagger → warn | NEST_SRC_GLOBS |
...
→ validation_whitelist/circular_deps fail 主触发；compliant 修正后全 pass。
-->
