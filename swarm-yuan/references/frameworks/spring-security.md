...
cmd: grep -rnE 'extends WebSecurityConfigurerAdapter' *.java
expect: hits>0
```
...
cmd: grep -rnE 'NoOpPasswordEncoder|Md5PasswordEncoder|MessageDigestPasswordEncoder|StandardPasswordEncoder|SHAPasswordEncoder|LdapShaPasswordEncoder' *.java
expect: hits>0
```
...
cmd: grep -rnE 'withDefaultPasswordEncoder' *.java
expect: hits>0
```
...
cmd: grep -rnE 'signWith\("|hmacShaKeyFor\("[^"]+"\.getBytes' *.java
expect: hits>0
```
...
cmd: grep -rnE 'csrf\(.*disable|AbstractHttpConfigurer::disable|\.csrf\(\)\.disable' *.java
expect: hits>0
```
...
cmd: grep -rnE '@(PreAuthorize|PostAuthorize)\([^)]*"' *.java | grep '\+'
expect: hits>0
```
...
cmd: grep -rnE 'has(Role|AnyRole)\("ROLE_' *.java
expect: hits>0
```
...
cmd: grep -rnE 'sessionFixation' *.java
expect: hits>0
```
...
cmd: grep -rlE '\.rememberMe\(|rememberMe\(' *.java
expect: hits>0
```
...
cmd: grep -rnE 'redirect-uri.*\*' *.yml *.yaml *.properties
expect: hits>0
```
...
cmd: grep -rnE 'antMatchers\(|\.authorizeRequests\(' *.java
expect: hits>0
```
