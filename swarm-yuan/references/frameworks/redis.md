...
| 门禁 id | 级别 | 实现逻辑 | 依赖变量 |
|---------|------|---------|---------|
| fw_redis_no_expire | fail | 二参 set 无 TTL 且同文件无 expire/entryTtl → fail | REDIS_SRC_GLOBS |
| fw_redis_lock | fail | 裸 setnx/setIfAbsent 无 owner+过期/Redisson 痕迹 → fail | REDIS_SRC_GLOBS |
| fw_redis_penetration | warn | get miss 回源无空值缓存/布隆痕迹 → warn | REDIS_SRC_GLOBS |
| fw_redis_breakdown | warn | 回源重建无互斥锁痕迹 → warn | REDIS_SRC_GLOBS |
| fw_redis_avalanche | warn | 常量 TTL 且无随机抖动痕迹 → warn | REDIS_SRC_GLOBS |
| fw_redis_serializer | warn | RedisTemplate Bean 无显式序列化器配置 → warn | REDIS_SRC_GLOBS |
| fw_redis_pipeline | warn | 循环体内逐条 get 无 multiGet/pipeline → warn | REDIS_SRC_GLOBS |
| fw_redis_key_naming | warn | 字符串字面量 key 无冒号分层 → warn | REDIS_SRC_GLOBS |
| fw_redis_pool | warn | spring redis 配置无 lettuce.pool → warn | REDIS_SRC_GLOBS |
| fw_redis_db_consistency | warn | DB 写 + RedisTemplate 同文件但无删缓存痕迹 → warn | REDIS_SRC_GLOBS |
...
fixture 验证覆盖：violating 含无 TTL set + 裸 SETNX 锁无 owner → fw_redis_no_expire/fw_redis_lock fail 主触发 + 循环逐条 GET；compliant 修正全 pass。
-->
