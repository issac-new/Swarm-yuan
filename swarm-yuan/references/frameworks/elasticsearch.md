...
| 门禁 id | 级别 | 实现逻辑 | 依赖变量 |
|---------|------|---------|---------|
| fw_es_deep_pagination | fail | from≥10000（Java/JSON）→ fail；from≥1000 → warn 提示 search_after | ES_SRC_GLOBS |
| fw_es_wildcard_prefix | fail | wildcard/query_string 查询值以 * 开头 → fail | ES_SRC_GLOBS |
| fw_es_refresh_interval | warn | 检出 bulk 写入但全仓无 refresh_interval → warn | ES_SRC_GLOBS |
| fw_es_bulk_backpressure | warn | BulkRequest 文件无 429 退避/重试/BulkIngester → warn | ES_SRC_GLOBS |
| fw_es_mapping_explosion | warn | 有 mappings 相关文件但无 total_fields.limit → warn | ES_SRC_GLOBS |
| fw_es_dynamic_mapping | warn | dynamic=true 显式检出，或 mappings 无 dynamic 声明 → warn | ES_SRC_GLOBS |
| fw_es_filter_context | warn | must 内放 term/terms/range/exists → warn | ES_SRC_GLOBS |
| fw_es_agg_depth | warn | 单文件 subAggregation ≥3 次 → warn 嵌套过深 | ES_SRC_GLOBS |
| fw_es_ilm | warn | 日期模式索引名但全仓无 ilm/lifecycle → warn | ES_SRC_GLOBS |
| fw_es_reindex_conflict | warn | _reindex 调用无 conflicts 声明 → warn | ES_SRC_GLOBS |
| fw_es_scroll_release | warn | scroll 使用无 ClearScroll → warn | ES_SRC_GLOBS |
| fw_es_version_compat | warn | RestHighLevelClient/include_type_name 命中 → warn 迁移 | ES_SRC_GLOBS |
| fw_es_connection_pool | warn | RestClient.builder 无连接池/超时配置 → warn | ES_SRC_GLOBS |

<!--
门禁 id 命名规范：fw_es_<rule>。
上表 13 条 id 在 assets/framework-gates/elasticsearch.sh 中均有同名实现；片段头 `# gates:` 与本表一致。
fixture 验证覆盖：violating 含 from(50000) 深分页 + wildcardQuery 前缀 * → fw_es_deep_pagination/fw_es_wildcard_prefix fail 主触发；compliant 用 search_after + filter 上下文精确查询 → 全 pass。
-->
