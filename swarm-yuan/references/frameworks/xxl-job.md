...
| fw_xxljob_access_token | fail | accessToken 为空/default_token/弱值 → fail；无配置 → warn | XXLJOB_SRC_GLOBS |
...
| fw_xxljob_glue_injection | fail | 动态执行 API + getJobParam 同文件 → fail；仅动态执行 → warn | XXLJOB_SRC_GLOBS |
