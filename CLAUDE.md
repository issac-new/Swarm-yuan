...
swarm-yuan 是**重量级范式**——20k 行文档 + 22k 行脚本 + 49 门禁 + 166 变量 + 62 框架规则。重量是设计选择不是缺陷：通过 `--profile auto|lite|standard|compliance` 四档让重量显式可选，`auto` 按项目规模+合规+技术栈复杂度自适应判定（质量优先升档偏置）。
