---
name: swarm-studio
description: SwarmStudio (hermes-studio v0.6.23) 二次开发全流程技能。覆盖 overlay/patch/inject 机制、cockpit/kanban/matrix-chat 组件开发、build:dmg 桌面打包、run-trace 执行链、url-guard 安全加固、embedded SQLite 持久化、hermes-studio-mcp 工具接入。工作目录 <project-root>/。
---

# Swarm-studio — SwarmStudio 二次开发需求交付全流程技能

> 本技能由 swarm-yuan 生成器（generate-skill.sh）创建骨架，已按 SwarmStudio 项目实际探查填充。
...
```bash
cd <project-root>/overlay          # 所有开发在此目录

# 环境与状态
bash .agents/skills/Swarm-studio/assets/env-setup.sh        # 检测 Node>=23 / Python>=3.11 / git/gh/docker
bash .agents/skills/Swarm-studio/scripts/state-machine.sh status   # 读当前阶段状态
...
> **gsd-core 安装器**只 prune `gsd-` 前缀目录；本技能（`Swarm-studio`，非 `gsd-` 前缀）在 `.agents/skills/` 下安全共存。
