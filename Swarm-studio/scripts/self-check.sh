...
check_superpowers(){
  if [[ -d ~/.claude/plugins/superpowers || -d ~/.agents/skills/superpowers ]]; then pass "superpowers: 已安装"; else miss "superpowers（需 /plugin install）"; fi
}
check_gstack(){
  if [[ -d ~/.agents/skills/gstack ]]; then pass "gstack: 已安装"; else miss "gstack（需 git clone + setup）"; fi
}
check_ruflo(){
  if command -v ruflo &>/dev/null; then pass "ruflo: $(ruflo --version 2>&1|head -1)"; else miss "ruflo"; fi
}
check_ecc(){
  if [[ -d ~/.claude/plugins/ecc || -d ~/.agents/skills/ecc ]]; then pass "ECC: 已安装"; else miss "ECC（需 /plugin marketplace add https://github.com/affaan-m/ECC && /plugin install ecc）"; fi
}
...
install_hint() {
  case "$1" in
    superpowers)
      ...
      ;;
    gstack)
      echo "    git clone --single-branch --depth 1 https://github.com/garrytan/gstack.git ~/.agents/skills/gstack"
      echo "    cd ~/.agents/skills/gstack && ./setup"
      ;;
    ECC)
