...
- **输出**：持久本地知识图（LadybugDB，存于 `.gitnexus/`，gitignored）；`AGENTS.md`/`CLAUDE.md` 上下文文件；`.agents/skills/gitnexus/` skill 文件；MCP server 暴露图谱查询工具
...
- **`--skills`**：Leiden 社区检测 → 每功能域一个 `SKILL.md`（`.agents/skills/generated/`）
